...
@Controller
@RequestMapping("/")
public class XuebaSsoController extends AbstractWebController {

    private final static String XUEBA_SOURCE = "17xueba";
    private final static String VALIDATE_URL = "/sso/ticket.vpage";
    ...
    @RequestMapping(value = "ssologin/17xueba.vpage", method = RequestMethod.GET)
    public String ssoLogin(Model model) {
        ...
        SsoConnections connectionInfo = SsoConnections.get(XUEBA_SOURCE);
        AbstractSsoConnector connector = ssoConnectorFactory.getSsoConnector(connectionInfo);
        if (connector == null) {
            String redirectUrl = UrlUtils.buildUrlQuery(ProductConfig.get17XuebaUrl() + "/sso/error.vpage", MiscUtils.m(
                    "uid", userId,
    ...
    private boolean validateTicket(Long userId, String ticket) {
        String validateUrl = ProductConfig.get17XuebaUrl() + VALIDATE_URL;
