    ...
    <div class="w-base" style="margin:-40px -20px -20px; border: none;">
        ...
        <div class="t-addstudent-poput" style="margin: 0;">
            <div class="t-student-create">
                <div class="main">
                    <div class="info">
                        <div class="box">给班内添加学生名单，可快速定位学生是否注册并成功加入班级。姓名必须为中文，且每行只能输入一个学生姓名。</div>
                    </div>
                    <div class="text" style="width: 240px; margin-left: 24px; position: relative; float: left; height:auto;">
                        ...
                        <textarea id="all_batch_student_name" class="w-int" style="width: 240px; height:350px;"></textarea>
                    </div>
                    <div class="t-important-clazz-info">
                        <p>添加学生方法：</p>
                        <p>方法1-手动输入：在左侧输入框内输入，每行限一个学生姓名。</p>
                        <p>方法2-Excel粘贴：详细步骤如下。</p>
                        <img style="display:block;width:100%;margin:10px 0;" src="<@app.link href='public/skin/teacherv3/images/publicbanner/important-clazz-info.png'/>">
                        <p>怎么把手中的Excel花名册学生名单导进来？<a href="http://help.17zuoye.com/?p=371" target="_blank" class="w-blue">点我帮你</a></p>
                    </div>
                </div>
            </div>
        </div>
...
<script type="text/javascript">
    $(function(){
        ...
        $(document).on("click", ".data-ImportStudentName", function(){
            ...
            currentId = $this.data("id");
            $.get("/teacher/clazz/clazzlist.vpage", {}, function(data){
                if(data.success){
                    ...
                    $.prompt(template("T:导入班级名单列表", {classMaps : _tempClassMaps, currentId : currentId}), {
                        ...
                        submit : function(e, v){
                            if(v){
                                ...
                                var studentNamesStr = _userNames.join(",");

                                App.postJSON("/teacher/clazz/importstudentnames.vpage", {
                                    clazzId : ${clazzId!0},
                                    studentNames : studentNamesStr
                                }, function(data) {
                                   if (data.success) {
                                       $("body").append("<iframe class='vox17zuoyeIframe' style='display:none;' src='/teacher/clazz/batchdownload.vpage?clazzIds=${clazzId!}'/>");
                                       $17.voxLog({
                                           module : "downloadStudentAccount",
                                           op : "import"
                                       });

                                       downloadSuccess("/teacher/clazz/batchdownload.vpage?clazzIds=${clazzId!}");
                                   } else {
                                       $17.alert(data.info || "导入学生名单失败");
                                   }
                                });
                            }
                        }
