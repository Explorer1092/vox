...
(function($17){
    ...
    function minusPlusInputEvent(opt, callback){
        var $defaultOpt = {
            ...
            multiple : 1,//倍数
            plus : ".vox-plus-btn",//加
            minus : ".vox-minus-btn",//减
            intCount : ".vox-count-int"//输入
        };
        ...
        if($defaultOpt.currentCount <= $defaultOpt.minCount){
            $($defaultOpt.minus).addClass("w-btn-disabled");
        }

        if($defaultOpt.currentCount >= $defaultOpt.maxCount){
            $($defaultOpt.plus).addClass("w-btn-disabled");
        }

        //减
        $($defaultOpt.box).on('click', $defaultOpt.minus, function(){
            var $this = $(this);
            var currentCountBox = $this.siblings("input");

            if($this.hasClass("w-btn-disabled")){
                return false;
            }

            var currentCount = parseInt(currentCountBox.val()) - $defaultOpt.multiple;

            if(currentCount <= $defaultOpt.minCount || !$17.isNumber(currentCountBox.val()) ){
                currentCount = 0;
            ...
            if(callback){
                $defaultOpt.currentCount = currentCount;
                callback($defaultOpt);
            }
        });

        //加
        $($defaultOpt.box).on('click', $defaultOpt.plus, function(){
            var $this = $(this);
            var currentCountBox = $this.siblings("input");
            ...
            if(currentCount >= $defaultOpt.maxCount || !$17.isNumber(currentCountBox.val()) ){
                currentCount = $defaultOpt.maxCount;
                $this.addClass("w-btn-disabled");
            }
        ...
        //输入个数
        $($defaultOpt.box).on('blur', '.v-count-int', function(){
            var $this = $(this);
            ...
            if(currentCount%$defaultOpt.multiple != 0){
                currentCount = $defaultOpt.multiple - currentCount%$defaultOpt.multiple + currentCount ;
                $this.val(currentCount)
            }

            if(currentCount < $defaultOpt.minCount || !$17.isNumber(currentCount)){
                $this.val($defaultOpt.minCount);
                $this.siblings($defaultOpt.minus).addClass("w-btn-disabled");
            }else if(currentCount >= $defaultOpt.maxCount){
                $this.val($defaultOpt.maxCount);
                $this.siblings($defaultOpt.plus).addClass("w-btn-disabled");
            }
...
window.onerror = function(errMsg, file, line){
    ...
    var url = 'http://log.17zuoye.cn/log?_c=vox_logs:js_errors_logs&_l=3&_log={"userId":"' + userId + '","errMsg":"' + errMsg + '","file":"' + file + '","line":"' + line + '","useragent":"' + useragent + '"}';
    ...
    window.voxLogger = {
        log: function(v){
            ...
            if(v.collection && v.collection != ""){
                ...
                $(function(){
                    var url = '//log.17zuoye.cn/log?_c=vox_flash:flash&_l=info&' + $.param(v) + '&uid=' + $.cookie('uid') + '&_t=' + new Date().getTime();
    ...
    districtSelect: {
        ...
        get                   : function(_this){
            ...
            if(next_level){
                ...
                $.getJSON('/getregion.vpage?regionCode=' + _this.val(), function(data){
