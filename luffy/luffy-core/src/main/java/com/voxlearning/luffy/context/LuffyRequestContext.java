...
package com.voxlearning.luffy.context;
 ...
 * @since 10/15/15
 */
public class LuffyRequestContext extends UtopiaHttpRequestContext {
    ...
    public LuffyRequestContext(HttpServletRequest request, HttpServletResponse response) {
    ...
    public Object getRootModelAttribute(String key) {
        switch (key) {
            ...
            case "currentUserType":
                Optional<User> user = getCurrentUserOptional();
    ...
    public Long getId(String param) {
        String _id = getRequest().getParameter(param);
        if (StringUtils.isBlank(_id)) {
            _id = getCookieManager().getCookie(param, "");
        }
        return SafeConverter.toLong(_id);
    }
