...
/**
 * BusinessDeveloperSalaryCalculator
 *
 * @author song.wang
 * @date 2016/9/19
 */
@Named
@Slf4j
public class BusinessDeveloperSalaryCalculator extends SalaryCalculator {
    ...
    private static final double[][] SCHEME_SS_CITY = {
            ...
            {0, 1050, 1200, 0, 900, 1012.5},
            {1500, 2100, 2400, 1275, 1800, 2025},
            {2250, 3150, 3600, 1912.5, 2700, 3037.5}
    };

    // S类城市提成方案
    private static final double[][] SCHEME_S_CITY = {
            {0, 0, 0, 0, 0, 0},      // 子数组长度为 6 ， 前三项表示直营模式的各项指标（小学单活， 小学双活， 中学单活），后三项表示代理模式的各项指标（小学单活， 小学双活， 中学单活）
            {0, 910, 1040, 0, 780, 877},
            {1300, 1820, 2080, 1105, 1560, 1755},
            {1950, 2730, 3120, 1658, 2340, 2632}
    };
    ...
    private static final double[][] SCHEME_B_CITY = {
            {0, 0, 0, 0, 0, 0},// 子数组长度为 6 ， 前三项表示直营模式的各项指标（小学单活， 小学双活， 中学单活），后三项表示代理模式的各项指标（小学单活， 小学双活， 中学单活）
            {0, 560, 640, 0, 480, 540},
            {800, 1120, 1280, 680, 960, 1080},
            {1200, 1680, 1920, 1020, 1440, 1620}
    };

    private static final double[] RANKING_SALARY = {10000, 8000, 6000, 4000, 2000};
    ...
    public void calculateCurrentEmployeeSalary(AgentUser user){
        ...
        calculateSalary(user, performanceData, cityLevelType, isJuniorAgentModel, isMiddleAgentModel, salaryStartDate, salaryEndDate);
    }
    ...
    public void calculateRankingSalary(AgentUser user){
        // 获取当前用户排名
        PerformanceRankingData rankingData = agentPerformanceRankingService.getRankingDataByUserId(user.getId(), 3, context.getRunDay());
    ...
    private double getRankingSalary(int ranking){
        ...
        }else if(ranking > 5 && ranking < 16){
            return RANKING_SALARY[1];
        }else if(ranking > 15 && ranking < 31){
            return RANKING_SALARY[2];
        }else if(ranking > 30 && ranking < 48){
            return RANKING_SALARY[3];
        }
//        else if(ranking > 50 && ranking < 52){
//            return RANKING_SALARY[4];
//        }
        return 0;
    }
