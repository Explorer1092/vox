...
function docReady(){
	...
	$('[data-rel="chosen"],[rel="chosen"]').chosen({
        ...
        placeholder_text_multiple : "请输入待查找选项"
    });
	...
	//datatable
    var unSorting = [] ;
    for (var i = 0;i< $('.datatable th').length ; i++){
        if($('.datatable th').eq(i).hasClass("unSorting")){
            unSorting.push(i);
        }
    }
	$('.datatable').dataTable({
			...
			"aoColumnDefs": [ { "bSortable": false, "aTargets": unSorting }],
			"oLanguage": {
                "sProcessing": "正在加载中......",
