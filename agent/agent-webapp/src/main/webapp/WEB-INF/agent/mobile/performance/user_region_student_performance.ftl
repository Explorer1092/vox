...
<ul class="mobileCRM-V2-list">
    <#if  regionPerformance?has_content>
        <#assign totaladdStuRegNum = 0>
        <#list regionPerformance as performance>
            <#assign addStuRegNum = performance.addStuRegNum!0>
            <#if performance.type == "REGION" || addStuRegNum gt 0>
                <#assign totaladdStuRegNum = totaladdStuRegNum + addStuRegNum>
                <li>
                    <#if performance.type != "SCHOOL">
                    <a href="/mobile/performance/user_region_student_performance.vpage?startDate=${startDate!''}&endDate=${endDate!''}&regionCode=${performance.key!''}" class="link link-ico">
                    <#else>
                    ...
                    <div class="side-fr side-orange side-width">${addStuRegNum!}</div>
                </a>
                </li>
            </#if>
        </#list>
        <li>
            <a href="javascript:void(0);" class="link linkNo-ico">
                ...
                <div id="sum-list" class="side-fr side-width side-total">${totaladdStuRegNum!}</div>
            </a>
        </li>
    </#if>
</ul>
...
<script type="text/javascript">
    $(function () {
        ...
        $("#select-ok").click(function () {
            ...
            window.location.href = "/mobile/performance/user_region_student_performance.vpage?startDate=" + startDate + "&endDate=" + endDate + "&regionCode=${regionCode!''}";
        });
    });
</script>
</@layout.page>
