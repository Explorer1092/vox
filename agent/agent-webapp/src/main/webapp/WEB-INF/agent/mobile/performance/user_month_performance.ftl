...
<div class="mobileCRM-V2-header">
    <div class="inner">
        <div class="box">
            ...
            <div class="headerText">高覆盖地区结算数</div>
        </div>
    </div>
</div>
...
<ul class="mobileCRM-V2-list">
    <#if regionPerformance?has_content>
        ...
        <#list regionPerformance as performance>
            <#assign num = performance.stuSlNum!0>
            <#assign total = total + num>
            <#if performance.type != "SCHOOL" || num gt 0>
                <li>
                    <#if performance.type != "SCHOOL">
                    <a href="/mobile/performance/user_month_performance.vpage?regionCode=${performance.key!''}&DATA=1&DATATYPE=${performance.type!''}&startDate=${startDate!''}&endDate=${endDate!''}" class="link link-ico">
                    <#else>
...
<script type="text/javascript">
    $(function () {
        ...
        $("#select-ok").click(function () {
            ...
            window.location.href = "/mobile/performance/user_month_performance.vpage?startDate=" + startDate + "&endDate=" + endDate + "&regionCode=${regionCode!''}&DATA=${data!0}&DATATYPE=${dataType!''}";
        });
    });
</script>
</@layout.page>