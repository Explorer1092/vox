    ...
    <div class="inner">
        <div class="box">
            <div class="branch_school_headerSearch">
                ...
                <div class="branch_school_box"><input type="text" placeholder="请输入学校名/ID" id="schoolKey" name="schoolKey" value="" class="branch_school_input"></div>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    $("#searchSchoolSubmit").on("click", function () {
        ...
        if (wd) {
            $.post("/mobile/school_clue/find_repeat_branch_school.vpage", {schoolKey: wd,schoolId:'${schoolId!''}'}, function (result) {
                console.log(result);
                if (result.success) {
