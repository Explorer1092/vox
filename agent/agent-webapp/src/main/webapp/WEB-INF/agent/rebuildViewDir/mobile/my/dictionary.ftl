...
<@layout.page title="字典表调整审核" pageJs="workflow" footerIndex=4>
                            ...
                            <div class="c-list">
                                <#list data as item>
                                    <div class="adjustmentExamine-box" style="margin-top: .5rem">
                                        <div class="adjust-head js-itemBtn" data-sid="${item.apply.id!0}" data-type="AGENT_MODIFY_DICT_SCHOOL" style="border:none">
                                            <div class="right">${item.apply.createDatetime?string("MM-dd")}</div>
                                            <div>字典表调整</div>
                                            <div class=""><#if item.apply.modifyType?? && item.apply.modifyType == 1>添加学校<#elseif item.apply.modifyType?? && item.apply.modifyType == 2>删除学校<#elseif item.apply.modifyType?? && item.apply.modifyType == 3>业务变更</#if>：${item.apply.schoolName!""}（${item.apply.schoolId!0}）</div>
