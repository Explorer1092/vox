...
<script type="text/javascript">
    ...
    var orgPostUrl = "uploadproductimage.vpage?productId=${(productId)!''}";
    var postUrl = orgPostUrl;

    $(document).ready(function(){
       $("select#attr-select").on("change",function(){
           // 先清空
          ...
          if($(this).val() == 'Sex'){
              ...
              $("select#value-select").change();
          }
       });

        $("select#value-select").on("change",function(){
            postUrl = orgPostUrl
                    + "&relateAttr=" + $("select#attr-select").val()
                    + "&relateValue=" + $(this).val();
            // 设置提交链接
            swfu.setUploadURL(postUrl);
        });

        $("select#attr-select").change();
    });
