...
<@layout_default.page page_title='资讯-新建/编辑内容' page_num=13>
...
<script>
    $(function () {
        ...
        $('#contentSubmitBtn').on('click', function () {
            ...
            //todo
            $.post('newarticle.vpage', {
                content: content,
                words_count: ue.getContentTxt().replace(/\n/g, '').length,
                ...
                rid: rid,
                content_by_album_news: new_content,
                video_url: video_url,
                video_img: $(".addBox img").data("file_name"),
                remark: $("#remark").val()
            }, function (data) {
                if (data.success) {
