...
<@layout_default.page page_title="校园大使注册反馈" page_num=3>
<div class="span9">
    <div>
        <form method="get" action="?" class="form-horizontal">
            <fieldset><legend>校园大使注册反馈</legend></fieldset>
            <ul class="inline form_datetime">
                <li>
                    <label for="userId">
                        处理状态
                        <select id="state" name="state">
                            <option value="1" <#if state == 1>selected </#if>>未处理</option>
                            <option value="2" <#if state == 2>selected </#if>>已处理</option>
                        </select>
                    </label>
                </li>
                <li>
        ...
        <table class="table table-striped table-bordered" style="font-size: 14px;">
            ...
            <#if feedbacks?has_content>
                <#list feedbacks as feedback>
                    <tr>
                        <td>${feedback.feedback.getCreateDatetime()?datetime!''}</td>
                        <td>${feedback.feedback.getCategory().getDescription()}</td>
                        <td><a href="../teacher/teacherhomepage.vpage?teacherId=${feedback.feedback.getUserId()!''}">${feedback.feedback.getUserId()!''}</a></td>
                        <td>${(feedback.feedback.getSensitiveMobile())!''}</td>
                        <td>${feedback.feedback.getContent()!''}</td>
                        <td>
                        ${feedback.feedback.getOperation()!''}
                        </td>
                        <td>
                            <#if feedback.feedback.getState() == 1>
                                未处理
                            <#else>
                                <#if feedback.feedback.getState() == 2>
                                    已处理
                                </#if>
                            </#if>
                        </td>
                        <td>
                            <#if feedback.feedback.getState() == 1>
                                <input name="btnprocess" type="button" value="处理" fid="${feedback.feedback.getId()!''}" />
                            </#if>
                        </td>
                    </tr>
                </#list>
...
<div id="alert_dialog" class="modal hide fade">
    ...
    <div class="modal-body dl-horizontal">
        您有<span id="alertcount"></span>条未处理请求！<a href="ambassadorindex.vpage">去处理</a></dt>
...
<script type="text/javascript">
    ...
    var modalshow = false;
    function alertform(){
        $.get('processambassadorcheck.vpage',function(data){
            if(data.success){
