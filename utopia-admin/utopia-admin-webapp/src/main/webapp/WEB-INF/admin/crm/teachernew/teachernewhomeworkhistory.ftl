...
<div style="margin-left: 2em" >
    ...
    <div style="margin-top: 2em">
        <ul class="inline">
            ...
            <li><form method="post" action="teachernewhomeworkhistory.vpage?homeworkDay=30&teacherId=${teacherSummary.teacherId!''}">
                <button  type="submit" <#if homeworkDay == 30 ||homeworkDay == 0>style="background-color: #C5C5C5"</#if>>查看30天内记录</button>
            </form></li>
            <li><form method="post" action="teachernewhomeworkhistory.vpage?homeworkDay=90&teacherId=${teacherSummary.teacherId!''}">
                <button  type="submit" <#if homeworkDay == 90>style="background-color: #C5C5C5"</#if>>查看3个月内记录</button>
            </form></li>
            <li><button  id="batchDeleteBtn">删除${homeworkDay}天内作业</button></li>
        </ul>
        ...
        <#if homeworkHistoryList?has_content>
            <#list homeworkHistoryList as homeworkHistory>
                <tr <#if (homeworkHistory["disabled"])>class="warning"</#if>>
                    ...
                    <td><a target="_blank"
                           href="../homework/newhomeworkhomepage.vpage?homeworkId=${homeworkHistory["homeworkId"]!''}">${homeworkHistory["homeworkId"]!''}</a>
                    </td>
                    <td><a target="_blank"
                    ...
                    <td>
                        ...
                        <#else>
                            <a name="homework_delete" role="button" data-content-id="${homeworkHistory["homeworkId"]!}"
                               class="btn">删除</a>
                            <a name="homework_add_Integral" role="button"
                               data-content-id="${homeworkHistory["homeworkId"]!}" class="btn">补发园丁豆</a>
                            <a name="integral_detail" role="button" data-content-id="${homeworkHistory["homeworkId"]!}"
                               data-type="homework" class="btn">园丁豆详情</a>
                        </#if>
                    </td>
                    <#if (homeworkHistory["isTermEnd"]=="TermReview")>
                        <td>期末作业</td>
                    <#else>
                        <#if (homeworkHistory["isTermEnd"]=="Similar")>
                            <td>类题作业</td>
                        <#else>
                            <td>普通作业</td>
                        </#if>
...
<script>
    $(function(){
        ...
        var teacherId = "${teacherSummary.teacherId!''}";
        $("#batchDeleteBtn").on("click",function () {
            if(confirm("确定删除老师ID为" + teacherId + "的作业吗？")){
                $.ajax({
                    type: "get",
                    url: "../teacher/batchdeletehomework.vpage",
                    data: {
                    ...
                    success: function (data) {
                        if (data.success) {
                            window.location.href = "teachernewhomeworkhistory.vpage?homeworkDay="+homeworkDay+"&teacherId=${teacherSummary.teacherId!''}";
                        } else {
                            alert(data.info);
                        }
                    }
        ...
        $("a[name='homework_delete']").on("click", function () {
            ...
            if(confirm("确定删除ID为" + homeworkId + "的作业吗？")){
                $.ajax({
                    type: "post",
                    url: "../teacher/disablenewhomework.vpage",
                    data: {
                    ...
                    success: function (data) {
                        if (data.success) {
                            window.location.href = "teachernewhomeworkhistory.vpage?homeworkDay=30&teacherId=${teacherSummary.teacherId!''}";
                        } else {
                            alert(data.info);
                        }
                    }
        ...
        $("a[name='homework_add_Integral']").on("click", function () {
            ...
            if(confirm("确定要补发园丁豆么？教师将按照完成学生数量的1.5倍发放，每个完成的学生将获得11学豆")){
                $.ajax({
                    type: "post",
                    url: "../teacher/addnewhomeworkintegral.vpage",
                    data: {
                        homeworkId: homeworkId,
                        teacherId: ${teacherSummary.teacherId!''}
                    },
        ...
        $("a[name='integral_detail']").on("click",function(){
            ...
            if(homeworkId != undefined &&  homeworkId !=""){
                $.ajax({
                    type:"post",
                    url:"newhomeworkgolddetail.vpage",
                    data:{
                        homeworkId:homeworkId,
                        teacherId:${teacherSummary.teacherId!''}
                    },
                    success:function(data){
                        if(data.success){
                            if(!data.isCheating){
                                ...
                                if(homeworkIntegral != undefined){
                                    $(".integralNum").text(homeworkIntegral);
