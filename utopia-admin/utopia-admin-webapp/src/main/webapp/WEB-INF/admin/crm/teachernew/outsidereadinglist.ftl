...
<div style="margin-left: 2em" >
    ...
    <#if (success)>
        <ul class="inline">
            <li><span style="font-weight:600">语文阅读任务历史</span></li>
        ...
        <table class="table table-hover table-striped table-bordered">
            ...
            <#if list?has_content>
                <#list list as reading>
                    <tr>
                        <td>${reading["id"]!''}</td>
                        <td>${reading["bookId"]!''}</td>
                        <td>${reading["bookName"]!''}</td>
                        <td>${reading["createAt"]!''}</td>
                        <td>${reading["endTime"]!''}</td>
                        <td>
                            <a name="homework_delete" role="button" data-content-id="${reading["id"]!''}" class="btn">删除</a>
                        </td>
                    </tr>
                </#list>
            <#else >
...
<script>
    $(function() {
        $("a[name='homework_delete']").on("click", function () {
            var readingId = $(this).attr("data-content-id");
            if (confirm("确定删除ID为" + readingId + "的阅读任务吗？")) {
                $.ajax({
                    type: "get",
                    url: "/crm/teachernew/deleteoutsidereading.vpage",
                    data: {
                        readingId: readingId
                    },
                    success: function (data) {
                        if (data.success) {
                            window.location.href = "/crm/teachernew/outsidereadinglist.vpage?groupId=${groupId!''}";
                        } else {
                            alert(data.info);
                        }
                    }
