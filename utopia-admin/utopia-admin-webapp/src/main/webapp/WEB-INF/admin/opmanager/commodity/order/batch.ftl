...
<div class="span9">
    <div>
        <form id="history-query" class="form-horizontal" method="get"
              action="${requestContext.webAppContextPath}/opmanager/commodity/order/batchList.vpage">
            <input type="hidden" id="pageNum" name="page" value="${currentPage!'1'}"/>
        </form>
        <span style="font-size: 24px">运单操作进度</span>
        <table id="rewards" class="table table-hover table-striped table-bordered" style="margin-top: 20px">
            ...
            <tbody>
                <#if batchPage?? && batchPage.content??>
                    <#list batchPage.content as mapper>
                        <tr>
                            <td>${mapper.opType!}</td>
                            <td>${mapper.fileName!}</td>
                            <td>${mapper.date!}</td>
                            <#if mapper.opType?? && mapper.opType == '批量导入'>
                                <td><#if mapper.url?? && mapper.url?length gt 0>有部分错误，请<a href="${mapper.url}">下载</a>查看<#else >导入完成</#if></td>
                                <#else >
                                <td>${mapper.url!''}</td>
                            </#if>
