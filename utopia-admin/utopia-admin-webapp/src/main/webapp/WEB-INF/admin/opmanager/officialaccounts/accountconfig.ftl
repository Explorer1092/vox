...
<@layout_default.page page_title="配置公众号投放对象" page_num=9>
...
<div id="main_container" class="span9">
    <legend class="legend_title">
        <strong>公众号概览</strong> <a type="button" style="margin-bottom: 5px;" class="btn" href="index.vpage">返回</a>
    </legend>
    <div class="inline">
        <table>
            <tbody><tr>
                <td class="info_td">公众号ID：<span class="info_td_txt">${accounts.id!0}</span></td>
                <td class="info_td">公众号名称：<span class="info_td_txt">${accounts.name!''}</span></td>
                <td class="info_td">公众号状态：
    ...
    <div class="form-horizontal">
        <legend class="legend_title"><strong>公众号投放对象</strong> &nbsp;&nbsp;&nbsp;&nbsp;</legend><br>
            ...
            <div>
                <table class="table table-stripped" style="width: 600px;" >
                    ...
                    <tr>
                        ...
                        <th <#if has_1?? && has_1> bgcolor="#00ffff" </#if>>
                            <input class="aaaa" type="radio" name="targetType" value=1 <#if targetType??><#if targetType == 1> checked="checked" </#if></#if>/>&nbsp;&nbsp;投放指定地区
                        </th>
                ...
                <div>
                    ...
                    <div id="target_label_modal" class="span7" style="display: none;">
                        <div id="labelTree" class="sampletree" style="width:60%; height: 410px; float: left; display: inline;"></div>
                        <div style="width:40%; height: 500px; float:right; display: inline;">
                            &nbsp;&nbsp;筛选 <input name="filter_label" type="text" class="input-small" id="filter_label" placeholder="筛选条件...">
                            <button name="delete_label_filter" id="delete_label_filter">&times;</button>
                            <br><br>&nbsp;&nbsp;<button class="btn btn-warning" onclick="checkUserHits()">关联用户数量</button>&nbsp;&nbsp;<input class="input-small" type="number" id="userCnt" disabled value="${labelUser!0}">
                            <#if accounts.status=="Offline">
                                <br><br>&nbsp;&nbsp;<button name="save_target_btn" type="button" class="btn btn-success">保存标签组</button>
                            ...
                            <table class="table table-striped table-bordered" style="width: 700px;">
                                <#if labelGroupList?? && labelGroupList?has_content>
                                    <tr><td>ID</td><td>标签组</td><td>操作</td></tr>
                                    <#list labelGroupList as label>
                                        <tr>
                                            <td>${label.targetId!'-'}</td>
                                            <td>${label.targetStr!""}</td>
                                            <td><button class="btn btn-danger" onclick="deleteLabel(${label.targetId})">删除</button></td>
                                        </tr>
                                    </#list>
                                </#if>
                            </table>
                    ...
                    <textarea type="text" id="targetUser" name="targetUser" class="form-control" rows="20"
                        ...
                        <div style="width:40%; height: 500px; float:right; display: inline;">
                            ...
                            <#if accounts.status=="Offline">
                                ...
                                <br><br>&nbsp;&nbsp;<button name="save_target_btn" type="button" class="btn btn-success">保存投放用户</button>
...
<script>
    $(function(){
        ...
        function initDisplay(targetType) {
            ...
            switch (type) {
                case 1: allModel.hide();regionModel.show();labelModel.hide();userModel.hide();schoolModel.hide();break;
//            case 4:  allModel.hide();regionModel.hide();labelModel.show();userModel.hide();schoolModel.hide();break;
                case 3:  allModel.hide();regionModel.hide();labelModel.hide();userModel.hide();schoolModel.show();break;
                case 2:  allModel.hide();regionModel.hide();labelModel.hide();userModel.show();schoolModel.hide();break;
                case 5:
                default: allModel.show();regionModel.hide();labelModel.hide();userModel.hide();schoolModel.hide();break;
            }
        }
    ...
    function deleteLabel(labelId) {
        $.post("deletelabel.vpage", {accountId: ${(accounts.id)!}, labelId: labelId}, function(data) {
            if (data.success) {
                alert("删除成功！");
                window.location.reload();
            } else {
