        ...
        <div class="span12">
            <form id="gameForm" class="well form-horizontal" method="post"
                            ...
                            <div class="controls">
                                <fieldset>
                                    <div class="control-group">
                                        ...
                                        <input type="text" required="required" placeholder="分组" class="input"
                                               name="group_name"/>
                ...
                <fieldset>
                    <legend>干扰项</legend>
                    <div class="control-group" data-type="data-item">
                        <fieldset data-type="options" data-label="选项">
                            <div class="control-group" data-type="data-item">
                                ...
                                <div class="controls">
                                    <fieldset>
                                        <div class="control-group">
                                            ...
                                            <input type="text" required placeholder="分组" class="input"
                                                   name="option_name"/>
                                            <button type="button" data-action="remove" class="btn-danger">移除</button>
                                        </div>
                                        <div class="control-group">
                ...
                <div class="control-group">
                    <fieldset>
                        <div class="control-label">
                            <button type="button" class="btn-success" data-action="append_options">新增干扰</button>
                        </div>
                    </fieldset>
                </div>
...
<script lang="javascript">
    $(document).ready(function () {
        ...
        var groupBinder = function (item, data) {
            ...
            item.find("[name='group_name']").val(data.name);
            item.find("[name='group_resource']").val(data.resource);
        }

        var optionBinder = function (item, data) {
            if (null == item || null == data) {
                return;
            }
            item.find("[name='option_name']").val(data.name);
            item.find("[name='option_resource']").val(data.resource);
        }
        ...
        var groupContext = null == context ? null : context.group;
        starter(group_container, uploader, group_append_button, groupBinder, groupContext);


        var option_container = $("[data-type='options']");
        var option_append_button = $("[data-action='append_options']");
        var optionContext = null == context ? null : context.option;

        starter(option_container, uploader, option_append_button, optionBinder, optionContext);
