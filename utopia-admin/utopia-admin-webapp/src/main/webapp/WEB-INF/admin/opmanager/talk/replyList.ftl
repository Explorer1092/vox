        ...
        <div class="span12">
            <form class="well form-horizontal" method="get" action="?">
                ...
                <ul class="inline">
                    <li>
                        <label>评论用户id</label>
                        <input type="text" name="userId" value="${userId!''}" placeholder="评论用户id"/>
                    </li>
                    <li>
                        <label>选择话题</label>
                            ...
                            <option value="">选择话题</option>
                                <#if topics?exists>
                                    <#list topics as topic>
                                        <option value="${topic.topicId!}" <#if topicId?exists && topicId == topic.topicId>selected</#if> >${topic.title!}</option>
                                    </#list>
                                </#if>
                        </select>
                    </li>
                    <li>
                        <label>精选话题</label>
                        <input type="checkbox" name="choice" <#if choice>checked</#if> value="1"/>
                    </li>
                    <li>
                        <label>存在回复</label>
                        <input type="checkbox" name="quoted" <#if quoted>checked</#if> value="1"/>
                    </li>
                </ul>
            ...
            <#if replies?exists>
                ...
                <tr class="info">
                    ...
                    <td data-status="${reply.auditStatus}" data-id="${reply.replyId}">
                        <button data-audit="pass" data-id="${reply.replyId}">通过</button>
                        <button data-audit="reject" data-id="${reply.replyId}">拒绝</button>
                        <br />
                        <button data-action="choice" data-value="${reply.choice}" data-id="${reply.replyId}"><#if reply.choice?exists && reply.choice == 1>取消精选<#else>设为精选</#if>
                        </button>
                        <a target="_blank" href="subReplyList.vpage?id=${reply.replyId!''}">查看回复</a>
                    </td>
                </tr>
                <#list reply.list as subreply>
                <tr>
                    <td>${subreply.replyId!''}</td>
                    <td>${subreply.concept!''}</td>
                    <td>${subreply.quote!''}</td>
                    <td>${subreply.createTime!''}</td>
                    <td>${subreply.topicTitle!''}</td>
                    <td>${subreply.userId!''}</td>
                    <td>${subreply.optionTitle!''}</td>
                    <td>${subreply.auditStatus!''}</td>
                    <td>${subreply.reason!''}</td>
                    <td>${subreply.auditTime!''}</td>
                    <td>${subreply.auditName!''}</td>
                    <td data-status="${subreply.auditStatus}" data-id="${subreply.replyId}">
                        <button data-audit="pass" data-id="${subreply.replyId}">通过</button>
                        <button data-audit="reject" data-id="${subreply.replyId}">拒绝</button>
                    </td>
                </tr>
                </#list>
                </#list>
...
<script lang="javascript">
    $(document).ready(function () {
        ...
        //设置精选
        $("button[data-action='choice']").click(function () {
            var replyId = $(this).attr("data-id");
            var obj = $(this);
            $.ajax({
                url: "choiceReply.vpage",
                type: "post",
                dataType: "json",
