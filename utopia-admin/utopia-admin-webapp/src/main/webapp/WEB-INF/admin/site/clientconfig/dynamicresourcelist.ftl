...
<div id="main_container" class="span9">
    ...
    <div>
        <form id="s_form" action="?" method="post" class="form-horizontal">
            <fieldset>
                <legend> 动态资源配置
                    <a href="toeditpage.vpage?type=DYNAMIC_APP_RESOURCE" type="button" class="btn btn-info" >添加</a>
                    <a href="totestpage.vpage?type=DYNAMIC_APP_RESOURCE" type="button" class="btn btn-warning">测试</a>
                </legend>
            </fieldset>
                        ...
                        <table class="table table-striped table-bordered" style=" white-space:nowrap; overflow:hidden;align=:center;">
                            ...
                            <#list patchList as patch >
                                <tr>
                                    ...
                                    <td><a href="toeditpage.vpage?type=DYNAMIC_APP_RESOURCE&id=${patch.id }">${patch.productId!}</a></td>
                                    <td>${patch.apkVer!}</td>
                                    <td>${patch.sysVer!}</td>
                                    <td>${patch.user!}</td>
                                    <td>${patch.model!}</td>
                                    <td>${patch.comment!}</td>
                                    <td>${patch.response.apkVer!}</td>
                                    <td>
                                        <#if (patch.isOpen)!false>启用
                                        <#else >禁用
                                        </#if>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <#if !patch.status?? || patch.status == 'draft'><a href="topublish.vpage?type=DYNAMIC_APP_RESOURCE&id=${patch.id }" type="button" class="btn btn-success">发布</a>
                                        <#elseif patch.status == 'published'><a href="todraft.vpage?type=DYNAMIC_APP_RESOURCE&id=${patch.id }" type="button" class="btn btn-danger">下线</a>
                                        </#if>
