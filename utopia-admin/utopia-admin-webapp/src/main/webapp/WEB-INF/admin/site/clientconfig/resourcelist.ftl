...
<div id="main_container" class="span9">
    ...
    <div>
        <form id="s_form" action="?" method="post" class="form-horizontal">
            <fieldset>
                <legend> App资源更新
                    <a href="toeditpage.vpage?type=APP_RESOURCE" type="button" class="btn btn-info" >添加</a>
                    <a href="totestpage.vpage?type=APP_RESOURCE" type="button" class="btn btn-warning">测试</a>
                </legend>
            </fieldset>
                        ...
                        <table class="table table-striped table-bordered" style=" white-space:nowrap; overflow:hidden;align=:center;">
                            ...
                            <#list patchList as patch >
                                <tr>
                                    ...
                                    <td><a href="toeditpage.vpage?type=APP_RESOURCE&id=${patch.id }">${patch.productId!}</a></td>
                                    <td>${patch.apkVer!}</td>
                                    <td>${patch.sysVer!}</td>
                                    <td>${patch.user!}</td>
                                    <td>${patch.model!}</td>
                                    ...
                                    <td>
                                        <#if !patch.status?? || patch.status == 'draft'><a href="topublish.vpage?type=APP_RESOURCE&id=${patch.id }" type="button" class="btn btn-success">发布</a>
                                        <#elseif patch.status == 'published'><a href="todraft.vpage?type=APP_RESOURCE&id=${patch.id }" type="button" class="btn btn-danger">下线</a>
                                        </#if>
