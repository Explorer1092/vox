            ...
            <form class="well form-horizontal" style="background-color: #fff;">
                <fieldset>
                    ...
                    <div class="control-group">
                        ...
                        <div class="controls">
                            <textarea name="comment" id="comment" style="height: 80px; width: 210px"><#if patch??>${patch.comment!''}</#if></textarea>
                        </div>
                    </div>
                    ...
                    <div class="control-group">
                        <label class="control-label">js文件链接(patchUrl)：</label>

                        <div class="controls">
                            <input type="text" id="patchUrl" name="patchUrl"
                                   <#if patch??>value="${patch.response.patchUrl!''}"</#if>/>
                            配置相对路径时请以"/"开头，如：/resources/app/jspatch/1.6.2.2_test.zip
                        </div>
                    </div>

                    <div class="control-group">
                        <label class="control-label">MD5值(patchMD5)：</label>

                        <div class="controls">
                            <input type="text" id="patchMD5" name="patchMD5"
                                   <#if patch??>value="${patch.response.patchMD5!''}"</#if>/>
                        </div>
                    </div>
    ...
    $(function () {
        ...
        $("#addFaqBtn").on("click", function () {
            ...
            var verMapper = {
                ...
                ktwelve: schools,
                user: $("#user").val(),
                patchUrl: $("#patchUrl").val(),
                patchMD5: $("#patchMD5").val(),
            ...
            appPostJson("addconfig.vpage", verMapper, function (data) {
                if (data.success) {
                    location.href = "list.vpage?type=" + "JSPATCH";
                } else {
                    alert(data.info);
                }
            });
