...
<@layout_default.page page_title='正式课用户答题结果查询' page_num=26>
...
<div class="main span10" id="search_result" v-cloak>
    <h3 style="padding: 0;margin: 0;">用户答题结果查询</h3>
    ...
    <span class="form-inline">
        <span>单元：</span>
        <select v-model="unit_index" @change="get_lessons">
                    ...
                    <template v-for="(row, i) in tableData">
                        ...
                        <tr v-if="row.schemaName === 'mock_qa'" v-for="(item, rowIndex) in row.answer">
                            ...
                            <td  style="vertical-align: top;">
                                <span v-if="row.activeServiceFlag" class="btn btn-success " @click="activeService(resultUserId,row.id,item.id)">主动服务</span>
                                <span  v-else class="btn btn-info " @click="activeService(resultUserId,row.id,item.id)">已服务</span>
                            </td>
                        </tr>
                        ...
                        <tr v-if="row.schemaName === 'word_repeat'" v-for="(item, rowIndex) in row.answer">
                            ...
                            <td style="vertical-align: top;">
                                <span v-if="row.activeServiceFlag" class="btn btn-success " @click="activeService(resultUserId,row.id,item.id,item.id)">主动服务</span>
                                <span v-else class="btn btn-info " @click="activeService(resultUserId,row.id,item.id)">已服务</span>
                            </td>
                        </tr>
                        ...
                        <#-- 角色扮演 - 视频对话 -->
                        <tr v-if="row.schemaName === 'role_play_practice'" v-for="(item, rowIndex) in row.answer">
                            <td v-if="rowIndex === 0" :rowspan="row.answer.length" style="vertical-align: top;">{{ row.customName || '/' }}</td>
                            <td v-if="rowIndex === 0" :rowspan="row.answer.length" style="vertical-align: top;">
                                <div v-if="row.question.jsonData.subjectId !== null" >
                                    {{ row.question.jsonData.translation || '/' }}
    ...
    var vm = new Vue({
        ...
        methods: {
            ...
            get_more_message: function() {
                ...
                $.post("/chips/user/question/index.vpage",{
                    userId:_this.user_id,
                    lessonId:_this.lesson_id
                }).then(function(res){
                    if(res.success) {
                        console.log(res)
                        _this.list = res.list;
            ...
            activeService:function (userId, qid, aid) {
                _this = this;
                $.get("/chips/ai/todaylesson/haveQuestionTemplate.vpage",{
                    qid:qid,
                }).then(function(res){
                    console.log("/chips/ai/todaylesson/userTemplateIndex.vpage?userId=" + userId + "&qid=" + qid
                            + "&bookId=" + _this.resultBookId + "&unitId=" + _this.resultUnitId + "&lessonId=" + _this.resultLessonId + "&aid=" + aid)
                    if(res.success) {
                        window.location.href = "/chips/ai/todaylesson/userTemplateIndex.vpage?userId=" + userId + "&qid=" + qid
                                + "&bookId=" + _this.resultBookId + "&unitId=" + _this.resultUnitId + "&lessonId=" + _this.resultLessonId + "&aid=" + aid;
                    }else {
                        alert( res.info);
                    }
