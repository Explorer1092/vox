...
public class CrmSchoolClueDao extends StaticMongoDao<CrmSchoolClue, String> {
    ...
    public List<CrmSchoolClue> findLocationSchoolClues(String schoolKey, Date updateStartTime, Date updateEndTime, Collection<Integer> checkStatus) {
        Filter filter = filterBuilder.where("authenticateType").is(5);
        filterIn(filter, "status", checkStatus);
        Long schoolId = SafeConverter.toLong(schoolKey);
        if (schoolId > 0) {
            filterIs(filter, "schoolId", schoolId);
        } else if (StringUtils.isNoneBlank(schoolKey)) {
            filterRegex(filter, "cmainName", schoolKey);
        }
        smartFilter(filter, "updateTime", updateStartTime, updateEndTime);
    ...
    private void filterIn(Filter filter, String key, Collection value) {
        if (CollectionUtils.isNotEmpty(value)) {
            filter.and(key).in(value);
        }
    }
