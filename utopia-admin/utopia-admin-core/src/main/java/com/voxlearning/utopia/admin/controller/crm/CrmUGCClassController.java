...
import com.voxlearning.utopia.admin.service.crm.CrmUgcClassService;
...
public class CrmUGCClassController extends CrmAbstractController {

    @Inject private RaikouSystem raikouSystem;
    @Inject private CrmUgcClassService crmUgcClassService;

    @RequestMapping(value = "classname_notonly.vpage")
    public String crmUgcClassNameNotOnly(Model model) {
        ...
        Page<CrmUGCClass> ugcClassList = crmUgcClassService.crmUgcClasses(triggerType, pageable);
        int page = getRequestInt("PAGE");
        model.addAttribute("page", page);
        model.addAttribute("ugcClassList", ugcClassList);
        model.addAttribute("triggerType", triggerType);
        return "crm/ugc/classname_notonly";
    }

    @RequestMapping(value = "classname_detail.vpage")
    public String crmUgcClassNameDetail(Model model) {

        Long groupId = getRequestLong("groupId");
        Long schoolId = requestLong("schoolId");
        int page = getRequestInt("PAGE");
        List<CrmUGCClassDetail> crmUGCClassDetailList = crmUgcClassService.crmUGCClassDetails(groupId);


        String sysClassName = crmUgcClassService.assembleClassName(groupId);
        School school = raikouSystem.loadSchool(schoolId);
        model.addAttribute("school", school);
        model.addAttribute("schoolId", schoolId);
        model.addAttribute("groupId", groupId);
        model.addAttribute("page", page);
        model.addAttribute("sysclassname", sysClassName);
        model.addAttribute("ugcClassDetailList", crmUGCClassDetailList);
        return "crm/ugc/classname_detail";
    }

    @RequestMapping(value = "updateugcclassname.vpage")
    public String updateUgcClassName(Model model) {

        Long schoolId = requestLong("schoolId");
        Long groupId = requestLong("groupId");
        int page = getRequestInt("PAGE");
        String updatedUgcClassName = requestString("updatedUgcClassName");
        crmUgcClassService.updateUgcClassName(schoolId, groupId, updatedUgcClassName);

        return redirect("classname_notonly.vpage??trigger=0&PAGE=" + page);

    }

    @RequestMapping(value = "ugcClassCount.vpage")
    @ResponseBody
    public MapMessage ugcClassNameCount() {
        MapMessage message = new MapMessage();
        long count = crmUgcClassService.getCrmUgcClassCount();
        message.add("classCount", count);
        return message;
    }

    @RequestMapping(value = "ugcClassDetailCount.vpage")
    @ResponseBody
    public MapMessage ugcClassNameDetailCount() {
        MapMessage message = new MapMessage();
        long count = crmUgcClassService.getCrmUgcClassDetailCount();
        message.add("classDetailCount", count);
        return message;
    }

    @RequestMapping(value = "exportUgcClassData.vpage")
    public void exportUgcClassData() {
        int size = getRequestInt("everySize");
        int page = getRequestInt("exportUgcClassPage");

        int skip = size * page;
        XSSFWorkbook xssfWorkbook = workbookClassNameData(size, skip);
        String fileName = "班级名称汇总情况.xlsx";
        writeExcel(xssfWorkbook, fileName);
    }

    @RequestMapping(value = "exportUgcClassDetailData.vpage")
    public void exportUgcClassDetailData() {
        int size = getRequestInt("everySize");
        int page = getRequestInt("exportClassDetailPage");

        int skip = size * page;
        XSSFWorkbook xssfWorkbook = workbookClassNameDetailData(size, skip);
        String fileName = "班级名称明细情况.xlsx";
        writeExcel(xssfWorkbook, fileName);
    }
    ...
    private XSSFWorkbook workbookClassNameDetailData(int size, int skip) {
        ...
        firstRow.createCell(2).setCellValue("GroupId");
        ...
        firstRow.createCell(7).setCellValue("ugc班级名称");
        ...
        List<CrmUGCClassDetail> crmUGCClassDetailList = crmUgcClassService.allCrmUgcClassDetailData(size, skip);


        for (CrmUGCClassDetail crmUGCClassDetail : crmUGCClassDetailList) {
            ...
            row.createCell(2).setCellValue(cellValue(crmUGCClassDetail.getGroupId()));
            row.createCell(3).setCellValue(cellValue(crmUgcClassService.assembleClassName(crmUGCClassDetail.getGroupId())));
            ...
            row.createCell(7).setCellValue(cellValue(crmUGCClassDetail.getUgcClassName()).replace("NULL", ""));
            row.createCell(8).setCellValue(cellValue(crmUGCClassDetail.getModUgcClassName()));
    ...
    private XSSFWorkbook workbookClassNameData(int size, int skip) {
        List<CrmUGCClass> crmUGCClassList = crmUgcClassService.allCrmUgcClassData(size, skip);

        XSSFWorkbook xssfWorkbook = new XSSFWorkbook();
        XSSFSheet sheet = xssfWorkbook.createSheet();
        XSSFRow headRow = sheet.createRow(0);
        headRow.createCell(0).setCellValue("班级名称汇总情况");
        ...
        firstRow.createCell(3).setCellValue("系统班级名称");
        firstRow.createCell(4).setCellValue("GroupId");
        ...
        for (CrmUGCClass crmUGCClass : crmUGCClassList) {
            ...
            row.createCell(4).setCellValue(cellValue(crmUGCClass.getGroupId()));
            ...
            row.createCell(10).setCellValue(cellValue(crmUGCClass.getUgcClassName()).replace("NULL", ""));
            row.createCell(11).setCellValue(percentFormat(crmUGCClass.getUgcAnswerPercent()) + "%");
            row.createCell(12).setCellValue(cellValue(crmUGCClass.getHistoryUgcClassName()).replace("NULL", ""));
