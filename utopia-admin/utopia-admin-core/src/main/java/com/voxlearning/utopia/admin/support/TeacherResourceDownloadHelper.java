...
@Named("com.voxlearning.utopia.admin.support.TeacherResourceDownloadHelper")
public class TeacherResourceDownloadHelper extends SpringContainerSupport {
    ...
    public DownloadContent downloadClazzStudentInformation(TeacherDetail teacher, List<Clazz> clazzs) {
        return this.downloadClazzStudentInformation(teacher, clazzs, "", "ACCOUNT");
    }
...
//            }
    ...
    private Map<String, Object> generateMapContent(ClazzInfoMapper mapper) {
        ...
        if (StringUtils.isBlank(SafeConverter.toString(map.get("mobile")))) {
            ...
            map.put("mobile", StringUtils.mobileObscure(getStudentParentMobile(parentList)));
        }
        return map;
    }
    ...
    private String getStudentParentMobile(List<StudentParent> studentParents) {
        ...
        if (keyParent != null) {
            authenticatedMobile = sensitiveUserDataServiceClient.showUserMobile(keyParent.getParentUser().getId(), "TeacherSystemClazzServiceImpl::getStudentParentMobile", SafeConverter.toString(keyParent.getParentUser().getId()));
        }
        if (authenticatedMobile == null || StringUtils.isEmpty(authenticatedMobile)) {
            // 没有关键家长，或关键家长手机未绑定，则随便一个绑定手机家长的手机
            for (StudentParent studentParent : studentParents) {
                authenticatedMobile = sensitiveUserDataServiceClient.showUserMobile(studentParent.getParentUser().getId(), "TeacherSystemClazzServiceImpl::getStudentParentMobile", SafeConverter.toString(studentParent.getParentUser().getId()));
                if (authenticatedMobile != null && StringUtils.isNotEmpty(authenticatedMobile)) {
                    break;
                }
            }
        }
        return authenticatedMobile != null ? authenticatedMobile : null;
    }
