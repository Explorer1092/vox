(function (root, factory) {
    ...
    var initEvent = function(instance, container, config){
        ...
        .on('uploadSuccess', function(file){
            ...
            config.onCustomUploadSuccess && typeof config.onCustomUploadSuccess == 'function' && config.onCustomUploadSuccess(file);
        })
        // .on('uploadSuccess', function(file){
        //     $('.uploader_info_success', container).text('');
        //     $('.uploader_info_error', container).html('上传失败，<a href="javascript:void(0);" class="uploader_reupload">重新上传</a>');
