...
<div class="bread-nav">
    ...
    &gt;
    <a class="current-dir" href="javascript:void(0);" style="cursor: default">编辑品牌</a>
</div>
<h3 class="h3-title">
    编辑品牌
...
<form id="add-form"   action="/basic/brand/save.vpage" method="post">
    <input type="hidden" name="id" value="${(mizarBrand.id)!}" />
    <div style="float:left;">
        <div class="input-control">
            <label><span class="red-mark">*</span>品牌名称：</label>
            <input id="brandName" name="brandName" data-title="品牌名称" class="require item"  value="${(mizarBrand.brandName)!''}" placeholder="请输入品牌名称"/>
        </div>

        <div class="input-control">
            <label>是否显示在列表：</label>
           <input type="checkbox" name="showList" id="showList"  data-title="showList" class="require checkTxt" value="1"  <#if (mizarBrand.showList)??><#if mizarBrand.showList==true>checked="checked"</#if></#if>">
        </div>
        <div class="input-control">
            <label>排序：</label>
            <input type="text" name="orderIndex" id="orderIndex" class="item" data-title="orderIndex" value="${(mizarBrand.orderIndex)!}" placeholder="请输入排序值">
        </div>

        <div class="input-control">
        <label>品牌规模：</label>
        <input id="shopScale" name="shopScale" data-title="品牌规模" class="item" value="${(mizarBrand.shopScale)!''}" placeholder="请输入品牌规模"/>
    </div>
        <div class="input-control">
            <label>品牌介绍：</label>
            <textarea name="introduction" id="introduction"  data-title="品牌介绍" placeholder="品牌介绍" style="resize: none;">${(mizarBrand.introduction)!''}</textarea>
        </div>
        <div class="input-control">
            <label>品牌特点：</label>
            <textarea name="points" id="points"  data-title="品牌特点" placeholder="品牌特点,逗号分隔" style="resize: none;" ><#if (mizarBrand.points)??>${(mizarBrand.points)?join(",")}</#if></textarea>
        </div>
        <div class="input-control">
            <label>获奖证书描述：</label>
            <textarea name="certificationName" id="certificationName"  data-title="获奖证书描述" placeholder="获奖证书描述" style="resize: none;" >${(mizarBrand.certificationName)!''}</textarea>
        </div>

        <div class="input-control">
            <label>创立时间：</label>
            <input name="establishment" id="establishment" class="item" data-title="创立时间" placeholder="创立时间"  value="${(mizarBrand.establishment)!}"/>
        </div>
        ...
        <div class="clearfix" style="clear:both;">
            ...
            <div class="image-preview brandLogo  clearfix">
                <#if (mizarBrand.brandLogo)??>
                    ...
                    <input  type="hidden" id="brandLogo"   name="brandLogo" value="${mizarBrand.brandLogo!}">
                     <img src="${mizarBrand.brandLogo!}" />
                    <div targetId="brandLogo" class="del-btn-img del-btn brandLogo">删除</div>
                    </div>
                </#if>
            </div>
        </div>
        <div class="clearfix" style="clear:both;">
            ...
            <div class="image-preview brandPhoto clearfix">
                <#if (mizarBrand.brandPhoto)??>
                    ...
                    <div id=img-preview-brandLogo class="image">
                        <input  type="hidden" id="brandPhoto"  name="brandPhoto" value="${brandPhoto!}">
                        <img src="${brandPhoto!}" />
                        <div targetId="brandPhoto" class="del-btn-img del-btn brandPhoto">删除</div>
                    </div>
                </#list>
              </#if>
            </div>
            ...
            <div class="image-preview certificationPhotos clearfix">
                <#if (mizarBrand.certificationPhotos)??>
                    <#list mizarBrand.certificationPhotos as certificationPhoto>
                        <div id=img-preview-brandLogo class="image">
                            <input  type="hidden" id="certificationPhotos"   name="certificationPhotos" value="${certificationPhoto!}">
                            <img src="${certificationPhoto!}" />
                            <div targetId="certificationPhoto" class="del-btn-img del-btn certificationPhoto">删除</div>
                        </div>
                    </#list>
                </#if>
            </div>
        ...
        <#--师资力量字段隐藏-->
        <div class="clearfix" style="clear:both; display: none;">
            <div class="input-control">
        ...
        <div class=" submit-box">
            ...
            <a id="abandon-btn" class="submit-btn abandon-btn" href="javascript:history.back();">返回</a>
        </div>
    </div>
</form>
