...
public class XssfUtils {
    ...
    public static final String XSSF_SURFIX = ".xlsx";
    ...
    public static void setCellValue(XSSFRow row, int column, CellStyle style, Float value) {
        Cell cell = row.getCell(column);
        if (cell == null) {
            cell = row.createCell(column);
        }
    ...
    public static String generateFilename(String... params) {
        return StringUtils.join(params, "-") + XSSF_SURFIX;
    }
    ...
    public static XSSFWorkbook convertToXSSFWorkbook(String[] titles, int[] width, List<List<String>> dataList) throws Exception {
        return convertToXSSFWorkbook(titles, width, dataList, "没有数据");
    }
