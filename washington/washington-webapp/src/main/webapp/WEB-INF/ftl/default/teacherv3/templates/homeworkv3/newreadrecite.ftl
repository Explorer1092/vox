<script id="t:NEW_READ_RECITE" type="text/html">
    <div class="md-course">
    ...
    <div class="aDetails-popup">
        <div class="r-inner showDetails">
            ...
            <div class="select-box">
                <div class="sel-option" data-bind="if:$root.showFilter('filter')">
                    <span data-bind="click:$root.changeQuestionType.bind($data,'important')">
                        <i data-bind="css:{'selted':$root.showImportantQuestion()}"></i>
                        重点段落
                    </span>
                    <span data-bind="click:$root.changeQuestionType.bind($data,'common')">
                        <i data-bind="css:{'selted':$root.showCommonQuestion()}"></i>
                        其他段落
                    </span>
                </div>
                <!--ko if: $root.currentQuestions_showCount() > 0-->
            ...
            <div class="readTxt-scrol">
                <div class="readTxt-details">
                    <!--ko if: $root.currentPageQuestions().length > 0-->
                    <!--ko foreach: $root.currentPageQuestions().slice(0,$root.showCount())-->
                    <div class="readInner" data-bind="style:{'display': $data.showQuestion() ? 'block' : 'none'}">
                        ...
                        <div class="r-playBtn">
                            ...
                            <span class="btn-option" style="cursor: pointer;" data-bind="click:$root.addQuestion.bind($data,$root)"><i></i>选入</span>
                            <!--/ko-->
                            <!--ko if: $data.questionChecked()-->
                            <span class="btn-option remove" style="cursor: pointer;" data-bind="click:$root.addQuestion.bind($data,$root)"><i></i>移除</span>
                            <!--/ko-->
                        </div>
                    </div>
                    <!--/ko-->
                    <!--/ko-->
                    <!--ko if: $root.currentQuestions_showCount() > 3 && $root.currentQuestions_showCount() > $root.showCount() -->
