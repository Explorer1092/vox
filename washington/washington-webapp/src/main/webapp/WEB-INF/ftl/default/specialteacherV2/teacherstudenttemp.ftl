...
<script id="manageMenuTemp" type="text/html">
    <div class="second-tab-box JS-teacStuManageContent">
        <!--ko foreach:menu -->
        <div class="second-tab-content bg-active JS-secTabContent" data-bind="css: {'color-active': $index() === 0}, click: $root.menuClick.bind($data,$index())">
            <i class="icon"></i>
            <a href="javascript:;"><span data-bind="text: $data"></span></a>
        </div>
        <!-- /ko -->
    </div>
</script>
            ...
            <div class="uploadBtn">
                <label>
                    <form action="/specialteacher/admin/batchimportteacher.vpage" method="POST" id="importTeacherForm" enctype="multipart/form-data">
                        <input type="file" class="v-fileupload v-fileupload1" accept=".xls, .xlsx" name="importTeacher" style="left:-9999px;position:absolute;"/>
                    </form>
...
<script id="createClazzModal" type="text/html">
    <div class="class-module mt-20 teacherManagement-box">
        ...
        <div class="management-right">
            <div class="uploadBtn">
                <label>
                    <form action="/specialteacher/admin/createteacherclazz.vpage" method="POST" id="teacherClazzForm" enctype="multipart/form-data">
                        <input type="file" class="v-fileupload v-fileupload2" accept=".xls, .xlsx" name="teacherClazz" style="left:-9999px;position:absolute;"/>
                    </form>
            ...
            <div class="uploadBtn">
                <label>
                    <form action="/specialteacher/admin/batchimportstudent.vpage" method="POST" id="importStudentForm" enctype="multipart/form-data" style="left:-9999px;position:absolute;">
                        ...
                        <#--默认disabled置为true不会提交，当第一次check返回klxDeleteSameName=true时表示存在可恢复学生，此时将disable置为false可提交，value值随用户按钮选择（value=true表示恢复，value=false表示不恢复）-->
                        <input type="text" name="recoverStudent" data-bind="attr: { disabled: studentSubmitRecoverDisabled, value: studentSubmitRecoverValue }" />
                    </form>
...
<#--添加学生账号时恢复弹窗-->
<script id="importStudentRestoreTemp" type="text/html">
    <div class="layer-module">
        <div class="layer-success">
            <div style="padding: 0 40px;">
                以下班级删除学生历史中与导入学生同名：<br/><span data-bind="html: importStudentRestoreInfo"></span><br/>是否恢复学生？
            </div>
        </div>
    </div>
</script>
