...
<#if (opts.getLink?has_content && opts.postLink?has_content)>
    ...
    <script id="T:UGC-RECORD" type="text/html">
        <style>
            ...
            .ugcDatetimeBtn{ background-position: right center;}
                ...
                <%for(var i = 0; i < items.length; i++){%>
                    ...
                    <%if(items[i].questionType === "INPUT" ){%>
                        <%if(items[i].questionName.indexOf("#INPUT_DATETIME#") > -1){%>
                            <p class="title"><%=(i+1)%>、<%==(items[i].questionName.replace(/#(INPUT_DATETIME)#/g, ' <input type="text" class="number ugcDatetimeBtn questionId-'+ items[i].questionId +'" placeholder="请选择时间" readonly="readonly"/> '))%></p>
                        <%}else{%>
    ...
    <script type="text/javascript">
        (function($){
            function isSubmitForm(){
                if( $17.getCookieWithDefault("ugcxxNst") ){
                    $(".js-ugcClickBtnPopup").show();
                }else{
                    $(".js-ugcClickBtnPopup").hide();
                }
            }

            //click popup file : dynamic.ftl
            isSubmitForm();

            function UgcRecordPopup(type){
                $17.setCookieOneDay("ugcxxAty", "1", 1);
                $.get("${(opts.getLink)!}?recordId=${(opts.recordId)!}", {}, function(data){
                    if(data.success && !$17.isBlank(data.ugc)){
                        recordMain(data);

                        //click popup
                        $17.setCookieOneDay("ugcxxNst", "1", 1);
                        isSubmitForm();
                    }else{
                        if(type == "click"){
            ...
            function recordMain(data){
                ...
                var statesHtml = {
                    state0 : {
                        focus: 1,
                        title : ( data.ugc.recordName ? data.ugc.recordName : "调查问卷" ),
                        position : { width: 570},
                        html : template("T:UGC-RECORD", {items : $unAnswerQuestions}),
            ...
            $.extend({
                UgcRecordPopup : UgcRecordPopup
            });

            $(document).on("click", ".js-ugcClickBtnPopup", function(){
                UgcRecordPopup("click");
            });
        }($));
    </script>
</#if>
