...
<ul class="breadcrumb_vox">
    ...
    <li class="active">区域统考</li>
    <li style="float: right"><a href="/rstaff/oral/index.vpage">返回</a></li>
</ul>
...
<script id="t:区统计数据" type="text/html">
    <tbody>
    <%
    ...
    %>
    <tr>
        <td><%=paperMapper[i].countyName%></td>
        ...
        <%
        if(paperMapper[i].county_name!="汇总"){
        %>
        <td class="text_blue paperName"><a style="text-decoration: underline;"
                                           href="/rstaff/oral/schoolStatistic.vpage?county_id=<%=paperMapper[i].countyId%>&paper_id=${paper_id}&exam_id=${exam_id}"
        >查看学校数据</a>
        </td>
        <%}else{%>
...
<script type="text/javascript">
    var paperOperate = {
        ...
        pagePaper: function (page) {
            ...
            paperOperate.$targetTable.removeClass("pageLoding").empty().html(template("t:区统计数据", {paperMapper: paperOperate.getContents()}));
        ...
        loadPaper: function (page) {
            ...
            App.postJSON("/rstaff/oral/oralRegionStatisticData.vpage", postData, function (data) {
