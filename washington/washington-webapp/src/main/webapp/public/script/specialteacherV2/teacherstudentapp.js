...
define(["jquery","knockout","YQ","knockout-switch-case","impromptu"],function($,ko,YQ){
    var teacherStuModal = function () {
        ...
        $.extend(self, {
            ...
            studentSubmitRecoverDisabled:ko.observable(true),
            studentSubmitRecoverValue:ko.observable(false),
            ...
            fileUploadBtn:function (formObj,index,data,event) {
                ...
                switch (index) {
                    ...
                    case 2:
                        ...
                        fileUrl = '/specialteacher/admin/checkteacherclazz.vpage';
                        break;
                    case 3:
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        self.lockUpload(false);
                        if (data.success) {
                            if (index === 3 && data.klxDeleteSameName) { // 添加学生可能需要展示是否恢复弹窗
                                self.studentSubmitRecoverDisabled(false); // recover input disabled to false and can submit.
                                self.fileUploadWithImportStuRestore(data.deleteSameStudents, formObj);
                            } else {
                                $(formObj).submit(); // 先执行js中的check接口，再执行temp中的form
                                if (index !== 1 && index !==2 && index !== 3){
                                    $.prompt("若导入的附件较大则需稍加等待。<br>导入结果文件会自动下载，请确保下载成功后关闭此弹窗。<br>操作成功后请在班级管理页面查看。", {
                                        focus: 0,
                                        title: "提示",
                                        buttons: {"确定": true}
                                    });
                                } else {
                                    $.prompt("若导入的附件较大则需稍加等待。<br>导入结果文件会自动下载，请确保下载成功后关闭此弹窗。", {
                                        focus: 0,
                                        title: "提示",
                                        buttons: {"确定": true}
                                    });
            ...
            // 添加学生账号 时展示可恢复弹窗
            fileUploadWithImportStuRestore: function (deleteSameStudents, formObj) {
                var ImportStuRestoreModal = function () {
                    var _self = this;
                    _self.importStuResoreData = ko.observable({
                        importStudentRestoreInfo: ko.observable('')
                    });
                };
                var importStuRestoreModal = new ImportStuRestoreModal();
                var importStuRestoreTempHtml = "<div id=\"importStuRestoreDialog\" data-bind=\"template: { name: 'importStudentRestoreTemp', data: importStuResoreData}\"></div>";
                $.prompt(importStuRestoreTempHtml, {
                    title: '系统提示',
                    focus: 1,
                    buttons: { "不恢复并继续": false, "恢复并继续": true},
                    position: {width: 640},
                    loaded: function () {
                        ko.applyBindings(importStuRestoreModal, document.getElementById('importStuRestoreDialog'));
                        importStuRestoreModal.importStuResoreData().importStudentRestoreInfo(self.exchangeDeleteSameStudentsDate(deleteSameStudents));
                    },
                    submit: function (e, v) {
                        e.preventDefault();
                        if (v) {
                            self.studentSubmitRecoverValue(true);
                        } else {
                            self.studentSubmitRecoverValue(false);
                        }
                        $(formObj).submit();
                        $.prompt.close();
                    }
            ...
            //通用函数，转换后端返回数据
            exchangeDeleteSameStudentsDate: function (data) {
                var collectArr = [];
                for (var sameStu in data) {
                    ...
                    collectArr.push("<span>" + sameStu + "，" + data[sameStu].length + "名学生（<span title='" + joinFullStr + "'>" + joinShortStr + "</span>）</span>");
    ...
    ko.applyBindings(tModal,document.getElementById("page_bd"));
});