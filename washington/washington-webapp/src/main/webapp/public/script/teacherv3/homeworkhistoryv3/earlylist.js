$(function(){
	function HomeworkList(option){
        ...
        self.begin = ko.observable(constantObj.defaultStartDate);
		...
		self.loadHomeworkList = function(){
			...
			$.get("/teacher/new/homework/report/oldhomeworklist.vpage",paramData,function(data){
				...
				if(data.success){
					...
					$17.voxLog({
						...
						op     : "API_STATE_ERROR",
						s0     : "/teacher/new/homework/report/oldhomeworklist.vpage",
						s1     : $.toJSON(data),
						s2     : $.toJSON(paramData),
						s3     : $uper.env
					});
	...
	//查询开始时间
	var endDate = new Date().toLocaleDateString().split('/').join('-');
    $( "#beginDateInput" ).datepicker({
        dateFormat      : 'yy-mm-dd',
        defaultDate     : constantObj.defaultStartDate,
        maxDate		  : endDate
    });
