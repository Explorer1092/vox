...
(function($17,ko) {
    ...
    IntelligentTeaching.prototype = {
                ...
                if(data.success){
                    ...
                    for(var z = 0, zLen = _realPackages.length; z < zLen; z++){
                        ...
                        _packages.push($.extend(true,_realPackages[z],{
                            ...
                            selCount : _boxQuestionMap[_realPackages[z].id] ? _boxQuestionMap[_realPackages[z].id].length : 0,
                            flag : "package",
                            totalCount : _realPackages[z].questions.length || 0,
                            teacherUsed : _realPackages[z].showAssigned || false,
        ...
        _resetExamList : function(_questions){
            ...
            var startPage = 1;
            var pageSize = self.pagination.fetchPageSize();
            var _startIndex = (startPage - 1) * pageSize;
            var _subExamQuestions = _questions.slice(_startIndex, _startIndex + pageSize);

            self.questionList = _questions;
            self.focusExamList(ko.mapping.fromJS(_subExamQuestions)());
            self.pagination.setPage(startPage,Math.ceil(_questions.length/pageSize));
        ...
        page_click : function(pageNo){
            ...
            var pageSize = self.pagination.fetchPageSize();
            var _startIndex = (pageNo - 1) * pageSize;
            self.focusExamList(ko.mapping.fromJS(self.questionList.slice(_startIndex,_startIndex + pageSize))());
        },
        ...
        viewCourse : function(){
            var self = this;
            var packageInfo = self.focusPackage();
            var data;
            switch (self.tabType) {
