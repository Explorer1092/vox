...
var homeworkReview = (function(constantObj){
            ...
            if(res.success){
                ...
                for(var k = 0,kLen = data.length; k < kLen; k++){
                    ...
                    if(question){
                        try{
                            _initVenus({
                                container: "#HR-" + questionId + "-" + k, //容器的id，（必须）
    ...
    var _initSubject = function(data,type){
        ...
        $17.QuestionDB.getQuestionByIds(questionIds,function(result){
            ...
            $.each(data,function(){
                var qid = this.id || this.questionId;
                var containerId = '#review_' + type + '_' + qid;
                //此处的样式与venusquestion.js文件统一
                $(containerId).css({"font-size":"18px","padding":"40px 20px"});
                _initVenus({
                    container: containerId, //容器的id，（必须）
                    formulaContainer:'#reviewhomework', //公式渲染容器（必须）
                    questionList: getQuestion(qid) //试题数组，包含完整的试题json结构， （必须）
                });

            });
        });
    };
    var _initVenus = function(options){
        var config = {
            container: null, //容器的id，（必须）
            formulaContainer:null, //公式渲染容器（必须）
            questionList: [], //试题数组，包含完整的试题json结构， （必须）
            framework: {
                vue: Vue, //vue框架的外部引用
        ...
        if(!config.container || config.questionList.length === 0){
            return false;
        }
    ...
    var _bind = function(carts){
        ...
        $(document).on("click",".single-dictation-question",function(e){
            ...
            $17.voxLog({
                module : "m_H1VyyebB",
                op : "pc_WD_preview_determines_page_single_word_check_remove_click",
                s0 : "ENGLISH",
                s1 : type,
                s2 : qid
            });
