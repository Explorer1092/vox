    ...
    IntelligentTeaching.prototype = {
        ...
        _fillFocusExamList: function(questions,callback){
            ...
            for(var m = 0; m < questions.length; m++){
                ...
                var postQuestions = questions[m].postQuestions;
                if($.isArray(postQuestions) && postQuestions.length > 0){
                    for(var j = 0,jLen = postQuestions.length; j < jLen; j++){
                        questionIds.push(postQuestions[j].id);
                    }
                }
            }
