!function() {
    ...
    var templateMap = {
        ...
        subjectInfo: ['<div class="h-set-homework examTopicBox" navtype="{navType}" qid="{qid}">',
                        ...
                        '</div>',
                        '<div class="seth-mn iconWrapper"><div id="subject_{qid}" style="padding: 20px 0 40px;"></div>{rate}<div class="testPaper-info"><div class="linkGroup"><a class="view_exam_answer" style="display: none;" href="javascript:void(0);">查看答案与解析</a></div></div></div>',
        ...
        subjectInfo_PHOTO_VOICE:['<div class="h-set-homework" qid="{qid}">',
                                    ...
                                    '<p class="fl"><span>{contentType}</span><span class="border-none">{difficulty}</span></p>',
                                    '</div>',
                                    '<div class="seth-mn iconWrapper line"><div id="subject_{qid}" style="padding: 20px 0 40px;"></div></div>',
                                    '{answers}',
                                    '</div>'].join(""),
    ...
    reportdetail.prototype = {
        ...
        initEvent: function(){
            ...
            $(".J_eventBind").on("click","a.view_exam_answer",function(){
                ...
                var gameUrl = "/teacher/new/homework/viewquestion.vpage?" + $.param({qids:_qid});
            ...
            $(".J_eventBind").on("click","li.mentalArithmeticTab",function(){
                ...
                if(elementId === "viewMentalQuestion" && !$element.hasClass("data-loaded")){
                        ...
                        if($.isArray(this.errorAnswerList) && this.errorAnswerList.length > 0){
                            if(this.showType==0){
                                $.each(this.errorAnswerList,function(i,item){
                                    ...
                                    answers += that.substitute(templateMap.answers_nopic,{odd:odd,answer:(that.optimizeAnswer(item.answer) || "未作答"),color:color,userName:userName.substring(0,userName.length-1)});
                                });
                                ...
                                $.each(this.errorAnswerList,function(i,item){
                                    ...
                                    if(navName=="PHOTO_OBJECTIVE"){
                                        ...
                                        answers += that.substitute(templateMap.answers_pic,{odd:odd,answer:(that.optimizeAnswer(item.answer) || "未作答"),color:color,picbox:picbox});
                                    }
                                    answers += "</ul></div>";
                                });
                    ...
                    var questionDetails = subjectInfos[navName].questionDetails || [];
                    var questionIds = [];
                    $.each(questionDetails,function(){
                    ...
                    case "WORD_TEACH_AND_PRACTICE":
                        $17.extend(viewModel,{
                            ...
                            viewModuleDetail  : function(stoneId,wordTeachModuleType){
                                //字词训练
                                switch (wordTeachModuleType){
                                    case "WORDEXERCISE":
                                        $17.voxLog({
                                            module : "m_Odd245xH",
                                            op : "class_result_word_question_detail_click",
                                    ...
                                    case "IMAGETEXTRHYME":
                                        $17.voxLog({
                                            module : "m_Odd245xH",
                                            op : "class_result_graphic_reading_detail_click",
                                            s0 : homework.clazzGroupId
                                        });
                                        break;
                                    case "CHINESECHARACTERCULTURE":
                                        $17.voxLog({
                                            module : "m_Odd245xH",
                                            op : "class_result_word_course_detail_click",
                                            s0 : homework.clazzGroupId
                                        });
                                        break;
                                    default:
                        ...
                        $17.voxLog({
                            module : "m_Odd245xH",
                            op : "class_result_word_training_load",
                            s0 : homework.clazzGroupId
                        });
                        break;
                    default:
