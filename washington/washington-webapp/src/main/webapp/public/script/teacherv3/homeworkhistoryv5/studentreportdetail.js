!function() {
    ...
    var templateMap = {
        ...
        subjectInfo: ['<div class="h-set-homework" qid="{qid}">',
                        ...
                        '</div>',
                        '<div id="subject_{qid}" style="font-size: 18px;padding: 20px 20px;" class="seth-mn"></div>{answers}',
                    '</div>'].join(""),
    ...
    stuReportDetail.prototype = {
        ...
        initDom : function(){
            ...
            if(typeTemplateMap.hasOwnProperty(navName)){
                ...
                var _initQuestionWithAnswer = function(questionIds,questionAnswerMap){
                    ...
                    $17.QuestionDB.getQuestionByIds(questionIds,function(result){
                        ...
                        $.each(studentInfos[navName],function(){
                            var questions = getQuestion(this.qid);
                            var questionAnswer = questionAnswerMap[this.qid] || {};
                            var answerResponse = {
                                userAnswers: questionAnswer["userAnswers"],
                                subMaster: questionAnswer["subMaster"],
                                master: questionAnswer["master"],
                            ...
                            var config = {
                                ...
                                questionList: questions, //试题数组，包含完整的试题json结构， （必须）
                                framework: {
                                    vue: Vue, //vue框架的外部引用
                                    vuex: Vuex //vuex框架的外部引用
                                },
                                showAnalysis: false, //是否展示解析
                                showUserAnswer: true, //是否展示用户答案
                ...
                $.get(student_history.getCompleteUrl+navName,{
                    studentId : student_history.studentId
                }).done(function(res){
                    _initQuestionWithAnswer(questionIds,res.result ? res.result : {});
                }).fail(function(){
                    _initQuestionWithAnswer(questionIds,{})
                });
            }
            $17.voxLog({
                module : "m_Odd245xH",
