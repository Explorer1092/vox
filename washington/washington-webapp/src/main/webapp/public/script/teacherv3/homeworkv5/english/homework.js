$(function(){
    ...
    constantObj._moduleSeconds = {
        ...
        VOICE_OBJECTIVE:0,
        DUBBING:0,
        DUBBING_WITH_SCORE:0,
        LEVEL_READINGS:0,
        INTELLIGENT_TEACHING :0,
        ORAL_INTELLIGENT_TEACHING : 0,
    ...
    var objectiveConfigType  = {
        ...
        ORAL_INTELLIGENT_TEACHING : "t:INTELLIGENT_TEACHING",
        ORAL_COMMUNICATION      : "t:ORAL_COMMUNICATION",
    ...
    levelAndclazzs.initialise({
        ...
        clazzClickCb  : function(){
            carts.resetClazzGroups(levelAndclazzs.getCheckedGroups());
    ...
    $("#saveHomworkBtn").on("click",function(){
        ...
        var _practices = constantObj._homeworkContent.practices,
            ...
            _tabDetails = [],
            _whiteTabList = ["BASIC_APP","LEVEL_READINGS","LS_KNOWLEDGE_REVIEW","NATURAL_SPELLING","PHOTO_OBJECTIVE","VOICE_OBJECTIVE","DUBBING","DUBBING_WITH_SCORE","ORAL_COMMUNICATION"],  //只要包含其中一项，则不检查题量多少
            _includeTabFlag = false, //是否包含白名单中的一项
            _questionCount = 0,
            groupNames = carts.getGroupDuration(levelAndclazzs.getCheckedGroups());

        for(var tab in _practices){
            if(_practices.hasOwnProperty(tab)){
                var _newContents = _practices[tab].questions || _practices[tab].apps;
                _newContents = _newContents || [];
                if($.isArray(_newContents) && _newContents.length > 0){
                    var _obj = {
                        tabType   : tab,
                    ...
                    _questionCount += _newContents.length;
        ...
        var popState = {
            ...
            state1 : {
                ...
                focus  : 1,
                position: { width : 500},
                html : "您本次练习题量过少，继续推荐将不会获得园丁豆！",
                buttons: { "继续推荐": true, "调整题目": false },
    ...
    $(".J_UFOInfo").on("click",".J_delete",function(e){
        ...
        if(constantObj._homeworkContent.practices[type].hasOwnProperty("papers")){
            constantObj._homeworkContent.practices[type].papers = [];
        }
        ...
        if($("#reviewhomework:visible")[0]){
            ...
            if(tabObj){
                if(type == "EXAM" || type == "WORD_PRACTICE"){
                    tabObj.clearAll();
                }else{
                    tabObj.run({
                        clazzGroupIdsStr : levelAndclazzs.checkedClazzGroupIds().join(",")
    ...
    ConfirmModule.prototype = {
            ...
            if(self.maxDurationMinutes != 0 && maxSeconds >= self.maxDurationMinutes * 60){
                var popState = {
                    state0 : {
                        ...
                        comment : '推荐练习',
                        html    : "<div class='w-ag-center'>建议练习时长不要超过" + self.maxDurationMinutes + "分钟</div>",
                        title   : '推荐练习',
                        position: { width : 450},
                        focus   : 1,
                        buttons: { "继续推荐": true, "返回调整": false },
                        submit  : function(e,v,m,f){
                            e.preventDefault();
                            if(v){
                    ...
                    if(limitClazzNames.length > 0 && limitTime > 0){
                        ...
                        var limitPopState = {
                            state0 : {
                                ...
                                comment : '推荐练习',
                                html    : "<div class='w-ag-center'>" + (clazzNamesTotal > 2 ? (joinLimitClazzNames + "等" + clazzNamesTotal + "个班级") : joinLimitClazzNames) + "已经有其它科目的练习正在进行，若继续推荐，学生的练习时长将大于" + Math.ceil(limitTime/60) + "分钟</div>",
                                title   : '推荐练习',
                                position: { width : 450},
                                focus   : 1,
                                buttons: { "继续推荐": true, "返回调整": false },
        ...
        _saveHomework      : function(element){
            var self = this,
                ...
                _homeworkContent = constantObj._homeworkContent;
            ...
            App.postJSON("/teacher/new/homework/assign.vpage", _homeworkContent, function(data){
                ...
                if(data.success){
                    ...
                    $17.alert("推荐成功",forwardHwList);
                   //  $17.homeworkv3 && $17.homeworkv3.teacherAppPopup("布置成功","m_H1VyyebB","popup_assign_confirmcontent_close_click",forwardHwList,constantObj.subject);
                }else{
                    $17.alert(data.info);
        ...
        initialise        : function(option){
            ...
            //todo 验证 currentDateTime 格式yyyy-MM-dd HH:mm:ss字符串
            //todo 验证 endDate,startDate 格式yyyy-MM-dd字符串
            self.startDateTime(option.startDateTime);
