!function() {
    ...
    stuList.prototype = {
        ...
        initDom : function(){
            ...
            $(".left-text").html((that.subject == "MATH" ? "数学" : that.subject == "CHINESE" ? "语文" : "英语") + "作业");
            $.post("/student/learning/history/newhomework/list.vpage",paramData,function(res){
                listDate = res;
                if(res.success){
                    that.mainContent();
                    that.initEvent();
            ...
            if(listDate.history && listDate.history.content && listDate.history.content.length>0){
                $.each(listDate.history.content,function(){
                    ...
                    switch (this.homeworkStatus){
                        case 0:
                            transfer = '<a href="javascript:void(0);" type="0" url="/student/learning/history/newhomework/homeworkreport.vpage?homeworkId='+this.homeworkId+'&subject='+that.subject+'" class="hw-btn hw-btn-green">查看作业详情</a>';
                            break;
                        case 1:
                            transfer = '<a href="javascript:void(0);" type="1" url="/student/homework/index.vpage?from=history&homeworkId=' + this.homeworkId + '" class="hw-btn hw-btn-yellow">立即补做</a>';
                            break;
                        case 2:
