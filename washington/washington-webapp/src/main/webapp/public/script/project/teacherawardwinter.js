    ...
    function teacherAwardMode () {
        ...
        _this.toAssign = function () {
            ...
            if (_this.isAssign()) {
                ...
                // 请求，一键布置作业
                $.ajax({
                    url: '/teacher/activity/winter/2017/onekey_assign.vpage',
                    type: 'GET',
                    dataType: 'json',
        ...
        if (getTime() < getTime('01/07/2018 23:59:59')) {
            _this.pageTipTitle('早布置福利');
            _this.pageTipContent('1月7日前布置的老师中必出1个3000元教育基金、4个1000元教育基金、5个kindle、20个100元教育基金，等你领取哦~');
        } else if (getTime() > getTime('01/08/2018 00:00:00') && getTime() < getTime('02/04/2018 23:59:59')) {
            _this.pageTipTitle('每周大奖');
            _this.pageTipContent('现在布置，2月4日前每周必出2个3000元教育基金、3个1000元教育基金、5个kindle、20个100元教育基金，等你领取~');
        } else if (getTime() > getTime('02/05/2018 00:00:00') && getTime() < getTime('02/18/2018 23:59:59')) {
            _this.pageTipTitle('最后一周');
            _this.pageTipContent('活动截止前布置，仍有机会领取1个3000元教育基金、4个1000元教育基金、5个kindle~');
        } else if (getTime() > getTime('02/19/2018 00:00:00')) {
            _this.pageTipTitle('活动已结束');
            _this.pageTipContent('当前活动已结束，请关注下次抽奖活动哦~');
        }
        ...
        function getDetailInfo () {
            $.ajax({
                url: '/teacher/activity/winter/2017/detail.vpage',
                type: 'GET',
                dataType: 'json',
                success: function (data) {
                    if (data.success) {
        ...
        function getHistoryInfo () {
            $.ajax({
                url: '/teacher/activity/winter/2017/history.vpage',
                type: 'GET',
                dataType: 'json',
                success: function (data) {
                    if (data.success) {
        ...
        $("#lotterySubmit").on('click', function () {
            ...
            if (_this.numTime() <= 0){
                if (!_this.isAssign()) { // 未布置
                    _this.showPopup(true);
                    _this.setAwardInfo(2, '当前剩余抽奖次数0次！', '布置假期作业，每天得3次抽奖机会~<br>假期作业默认开始时间1月22日，<br>不用担心学生提前开始。', '一键布置');
                } else { // 已布置
                    _this.showPopup(true);
        ...
        function lotteryPost($this){
            $.ajax({
                url: "/teacher/activity/winter/2017/draw.vpage",
                type: "POST",
                dataType: "json",
                data:{
                    ...
                    if (data.success) {
                        ...
                        lotteryAnimate = setInterval(function () {
                            startRotational(function () {
                                ...
                                switch (winningId) {
                                    case 1:
                                        _this.setAwardInfo(4, '恭喜你获得3000元教育基金！', '教育基金将以京东卡的形式发放，在3月5日之后统一寄送，请注意查收电话哦~', '知道了');
                                        break;
                                    case 2:
                                        _this.setAwardInfo(4, '恭喜你获得1000元教育基金！', '教育基金将以京东卡的形式发放，在3月5日之后统一寄送，请注意查收电话哦~', '知道了');
                                        break;
                                    case 3:
                                        _this.setAwardInfo(5, '恭喜你获得Kindle一台！', '实物奖品3月5日之后统一寄送，请注意查收电话哦~', '知道了');
                                        break;
                                    case 4:
                                        _this.setAwardInfo(4, '恭喜你获得100元教育基金！', '教育基金将以京东卡的形式发放，在3月5日之后统一寄送，请注意查收电话哦~', '知道了');
                                        break;
                                    case 5:
