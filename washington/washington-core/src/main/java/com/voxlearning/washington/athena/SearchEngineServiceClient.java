 ...
 * @since 2016/6/15
 */
@Named("com.voxlearning.washington.athena.SearchEngineServiceClient")
public class SearchEngineServiceClient implements InitializingBean {
    ...
    public List<LabelDataObject> getLabelTree() {
        try {
            ...
            if (labelData == null) {
                labelData = remoteReference.getLabelData();
    ...
    public Set<String> getUserLabelSet(Long userId) {
        try {
            ...
            if (userLabel == null) {
                userLabel = remoteReference.getTags(Collections.singleton(String.valueOf(userId))).get(userId);
                if (CollectionUtils.isNotEmpty(userLabel)) {
                    userLabel = userLabel.stream()
                            .map(val -> SafeConverter.toLong(val) & LABEL_FILTER_FLAG)
                            .map(SafeConverter::toString)
                            .collect(Collectors.toSet());
                } else {
                    userLabel = Collections.emptySet();
                }
    ...
    private List<LabelDataObject> loadLabelDataFromCache() {
        String cacheKey = CacheKeyGenerator.generateCacheKey(LABEL_CACHE_KEY, new String[]{"T"}, new Object[]{DateUtils.dateToString(new Date(), "yyyyMMdd")});
    ...
    private Map<Long, String> loadLabelMapFromCache() {
        String cacheKey = CacheKeyGenerator.generateCacheKey(LABEL_CACHE_KEY, new String[]{"M"}, new Object[]{DateUtils.dateToString(new Date(), "yyyyMMdd")});
    ...
    private Set<String> loadUserLabelFromCache(Long userId) {
        String cacheKey = CacheKeyGenerator.generateCacheKey(LABEL_CACHE_KEY, new String[]{"U"}, new Object[]{userId});
