...
@PubsubSubscriber(destinations = {
        @PubsubDestination(system = PubsubSystem.KFK, connection = "primary", topic = "platform.queue.parent.homework.app.use.topic"),
        @PubsubDestination(system = PubsubSystem.KFK, connection = "main-backup", topic = "platform.queue.parent.homework.app.use.topic")
}, maxPermits = 64)
public class PracticeHomeWorkListener implements MessageListener {
    ...
    public void onMessage(Message message) {
        ...
        //获取学生ID
        Long sid = SafeConverter.toLong(map.get("userId"));
        if (null == sid || 0 == sid) {
            return;
        }
        ...
        if ("EXAM".equals(bizType)) {
            bizType = "PARENT_HOMEWORK_ENTRY_NEW";
        }
        if ("MENTAL_ARITHMETIC".equals(bizType)) {
            bizType = "PARENT_QUICK_ORAL_ENTRY_NEW";
        }
        if ("OCR_MENTAL_ARITHMETIC".equals(bizType)) {
            bizType = "PARENT_ORAL_EXERCISE_ENTRY_NEW";
        }
        if ("INTELLIGENT_TEACHING".equals(bizType)) {
            bizType = "PARENT_MATH_PRACTICE_ENTRY_NEW";
        }

        //获取消息类型
        String messageType = SafeConverter.toString(map.get("messageType"));
        if (StringUtils.isBlank(messageType) || !"use".equals(messageType)) {
            return;
        }
