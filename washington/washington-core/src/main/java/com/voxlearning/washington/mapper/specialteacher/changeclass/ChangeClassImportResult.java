 ...
 * @since 2017/7/13
 */
public class ChangeClassImportResult {
    public List<ChangeClassImportData> successList;
    public List<ChangeClassImportData> failedList;

    public ChangeClassImportResult() {
        successList = new ArrayList<>();
        failedList = new ArrayList<>();
    }

    public void success(ChangeClassImportData data) {
        successList.add(data);
    }

    public void failed(ChangeClassImportData data) {
        failedList.add(data);
    }
    ...
    public List<ExcelExportData> toExportExcelData() {
        ...
        String[] successTitle = new String[]{"年级", "学生姓名", "换班后新班级", "成功/失败（操作后回填）", "备注（操作后回填）"};
        int[] successWidth = new int[]{4000, 4000, 4000, 4000, 8000};

        List<List<String>> successData = successList.stream()
                .map(data -> Arrays.asList(data.getGradeName(), data.getStudentName(), data.getTargetClazz(), data.getResult(), data.getRemark()))
                .collect(Collectors.toList());

        exportData.add(new ExcelExportData("成功列表", successTitle, successWidth, successData, 5));

        String[] failedTitle = new String[]{"年级", "学生姓名", "换班后新班级", "成功/失败（操作后回填）", "备注（操作后回填）"};
