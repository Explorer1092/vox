...
@Controller
@RequestMapping(value = "/student/voice/")
@Slf4j
public class StudentVoiceEngineController extends AbstractController {
    ...
    public MapMessage voiceScore() {
        ...
        if (MapUtils.isEmpty(map)) {
            return MapMessage.errorMessage(RES_RESULT_DATA_ERROR_MSG);
        }

        User user = currentUser();
        if (user == null || user.getId() == null) {
            return MapMessage.errorMessage(RES_RESULT_USER_ACCOUNT_NOT_EXIST_MSG);
        }
        ...
        try {
            ...
            if (MapUtils.isEmpty(resultMap) || resultMap.get("score_json") == null || StringUtils.isBlank((String) resultMap.get("score_json"))) {
                return MapMessage.errorMessage(RES_RESULT_SCORE_IDS_ERROR);
            }
            ...
            return MapMessage.errorMessage(RES_RESULT_DUPLICATE_OPERATION);
        } catch (RuntimeException ex) {
            logger.warn("Upload voice failed writing: (userId={})", user.getId(), ex.getMessage());
            return MapMessage.errorMessage("打分异常");
        } catch (Exception ex) {
            logger.warn("Upload voice failed writing: (userId={})", user.getId(), ex.getMessage());
            return MapMessage.errorMessage(RES_SUBJECTIVE_UPLOAD_FAIL_MSG);
        }
        ...
        return MapMessage.successMessage().add(RES_VOICE_SCORE_INFO, resultMap);
    }
