...
public class ForgotPasswordMobileController extends AbstractController {
    ...
    public MapMessage mobileResetPasswordStep1() {
        ...
        String code = RandomGenerator.generateAuthenticationCode();
        MobileResetPasswordContext context = MobileResetPasswordContext.init(users, mobile, code, "");
    ...
    public MapMessage mobileResetPasswordStep3() {
        ...
        if (!StringUtils.equals(context.mobileLastCode, code)) return MapMessage.errorMessage("验证码不匹配");

        context.verified = true;
        // 验证成功, 追踪，记录验证码已经消费掉
        if (!StringUtils.isEmpty(context.trackId)) {
            smsServiceClient.getSmsService().updateSmsConsumed(context.trackId);
        }
        saveContext(context);
        return MapMessage.successMessage().add("token", context.token).add("mobile", context.mobile);
    }
    ...
    public MapMessage mobileResetPasswordStep4() {
        ...
        userServiceRecord.setComments("用户重置密码，操作端app");
    ...
    public MapMessage sendVerificationCode() {
        try {
            ...
            if (StringUtils.isEmpty(context.mobileLastCode)) {
                context.mobileLastCode = RandomGenerator.generateAuthenticationCode();
            }
    ...
    private void saveContext(MobileResetPasswordContext context) {
        washingtonCacheSystem.CBS.flushable.set("MobileResetPassword:" + context.token, 3600, context);
    }

    private MobileResetPasswordContext getContext(String token) {
        return washingtonCacheSystem.CBS.flushable.load("MobileResetPassword:" + token);
    }

    private void destroyContext(MobileResetPasswordContext context) {
        washingtonCacheSystem.CBS.flushable.delete("MobileResetPassword:" + context.token);
    }
