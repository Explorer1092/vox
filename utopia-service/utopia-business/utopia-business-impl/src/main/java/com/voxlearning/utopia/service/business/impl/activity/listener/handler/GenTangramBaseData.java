...
@Named
public class GenTangramBaseData extends AbstractGenBaseData {

    private static final Logger logger = LoggerFactory.getLogger(GenTangramBaseData.class);
    ...
    public void execute(ReportContext reportContext) {
        ...
        for (int i = 0; i < totalPages; i++) {
            ...
            Page<TangramEntryRecord> entryRecords = studentActivityServiceClient.loadTangramRecordPage(activity.getId(), i, 30000);
            totalPages = entryRecords.getTotalPages();
            List<ActivityReportBaseData> baseData = genTangramActivityReportData(reportContext, entryRecords.getContent(), activity.getId());
            Map<String, String> map = genTangramActivityReportExcel(baseData, genFileName(activity.getId(), totalPages, i + 1));
    ...
    public List<ActivityReportBaseData> genTangramActivityReportData(ReportContext context, List<TangramEntryRecord> tangramEntryRecords, String activityId) {
        ...
        for (TangramEntryRecord tangramEntryRecord : tangramEntryRecords) {
            try {
                userActivityReportBaseDataList.clear();

                Long userId = tangramEntryRecord.getUserId();
                ...
                Map<Long, Integer> scoreMap = tangramEntryRecord.getScoreMap();
                if (MapUtils.isNotEmpty(scoreMap)) {
                    ...
                    while (it.hasNext()) {
                        ...
                        activityReportBaseData.setActivityType(ActivityTypeEnum.TANGRAM.name());
                ...
                logger.error("七巧板活动数据" + tangramEntryRecord.getId() + "生成活动报告基础异常", e);
            }
        }
        return activityReportBaseDataList;
    }

    public Map<String, String> genTangramActivityReportExcel(List<ActivityReportBaseData> allActivityReportBaseDatas, String fileName) {
        Map<String, String> result = new HashMap<>();

        String[] titles = new String[]{"日期", "地区", "学生ID", "学生姓名", "学校ID", "学校名称", "班级ID", "年级", "班级", "成绩"};
    ...
    private void writeLine(XSSFSheet sheet, int i, ActivityReportBaseData data) {
        ...
        dataCell1.setCellValue(data.getRegionName());
        ...
        dataCel14.setCellValue(data.getSchoolId());
        XSSFCell dataCel15 = dataRow.createCell(5);
        dataCel15.setCellValue(data.getSchoolName());
        XSSFCell dataCel16 = dataRow.createCell(6);
        dataCel16.setCellValue(data.getClazzId());
        XSSFCell dataCel17 = dataRow.createCell(7);
        dataCel17.setCellValue(ClazzLevel.getDescription(data.getClazzLevel()));
        XSSFCell dataCel18 = dataRow.createCell(8);
        dataCel18.setCellValue(data.getClazzName());
        XSSFCell dataCel19 = dataRow.createCell(9);
        dataCel19.setCellValue(data.getScore());
    }
