...
@Named
@Slf4j
public class TeacherTaskPrimaryRookieInit extends AbstractTeacherTaskInit implements InitializingBean {
    ...
    public List<TeacherSubTaskEntry> getTeacherSubTaskEntry(TeacherDetail teacherDetail, TeacherTask teacherTask, TeacherTaskProgress teacherTaskProgress) {
        ...
        for (TeacherSubTaskEntry teacherSubTaskEntry : subTaskEntryList) {
            ...
            if (Objects.equals(teacherSubTaskEntry.getId(), 2L)) {
                ...
                TeacherTaskProgress.Reward reward = rewardMap.values().stream().filter(r -> Objects.equals(r.getUnit(), TeacherTaskTpl.RewardUnit.exp.name())).findAny().orElse(null);
                if (reward != null) {
                    rewards.add(new TeacherSubTaskEntry.Reward(2L, TeacherTaskTpl.RewardUnit.exp.name(), reward.getValue(), 2, ""));
                }
            }
    ...
    public TeacherTaskTpl.Tpl getTeacherTaskTpl() {
        return TeacherTaskTpl.Tpl.PRIMARY_ROOKIE;
    }
}