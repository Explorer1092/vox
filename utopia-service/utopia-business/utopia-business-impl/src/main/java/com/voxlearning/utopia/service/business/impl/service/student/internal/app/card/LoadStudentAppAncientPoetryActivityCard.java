 ...
 */
@Named
public class LoadStudentAppAncientPoetryActivityCard extends AbstractStudentAppIndexDataLoader {
        ...
        if (groupMapper != null) {
            ...
            if (message.isSuccess()) {
                ...
                for (Map<String, Object> resultMap : result) {
                    ...
                    if (!passed) {
                        Map<String, Object> card = generateCard((String) resultMap.get("activityId"), desc, context.sys);
    ...
    private Map<String, Object> generateCard(String activityId, String desc, String sys) {
        ...
        // 安卓endDate可以为null, ios的endDate可以为空字符转
        if (StringUtils.equalsIgnoreCase("ios", sys)) {
            cardMap.put("endDate", "");
        }
        cardMap.put("types", Collections.singletonList("CHINESE_READING"));
        cardMap.put("url", UrlUtils.buildUrlQuery(NewHomeworkConstants.STUDENT_ANCIENT_POETRY_ACTIVITY_URL, MapUtils.m("activityId", activityId, "fullscreen", true)));
        return cardMap;
    }
}
