 ...
 */
@UtopiaCachePrefix(prefix = "PICBOOK_STUDENT_TOP100_RANK")
@UtopiaCacheExpiration(policy = CachedObjectExpirationPolicy.fixed, value = 86400 * 14)
public class StudentPicBookTopRankCacheManager extends PojoCacheObject<StudentPicBookTopRankCacheManager.GeneratorKey, Map<Long, PicBookRankInfo>> {

    public StudentPicBookTopRankCacheManager(UtopiaCache cache) {
        super(cache);
    }

    public List<PicBookRankInfo> fetchStudentRankList(PicBookRankCategory rankType, Integer weekRange) {
        if (rankType == null)
            return Collections.emptyList();
        try {
            Map<Long, PicBookRankInfo> maps = load(new GeneratorKey(rankType, weekRange));
            if (maps == null)
                return Collections.emptyList();
    ...
    public Integer getStudentRank(PicBookRankCategory rankType, Long studentId, Integer weekRange) {
        if (rankType == null || studentId == null)
            return 0;
        try {
            Map<Long, PicBookRankInfo> maps = load(new GeneratorKey(rankType, weekRange));
    ...
    public void updateStudentToRank(PicBookRankCategory rankType, PicBookRankInfo rankInfo, Integer weekRange) {
        if (rankType == null || rankInfo == null || rankInfo.getStudentId() == null)
            return;

        String key = cacheKey(new GeneratorKey(rankType, weekRange));
        ...
        if (null != cacheObject.getValue()) {
            cache.cas(key, expirationInSeconds(), cacheObject, 3, currentValue -> {
                ...
                if (currentValue.containsKey(rankInfo.getStudentId())) {
                    ...
                    // 如果不存在，当超过150长度时,删除元素为新元素腾出空间(防止有并列产生,保存到150人)
                    List<PicBookRankInfo> rankList = new ArrayList<>(currentValue.values());
                    if (rankList.size() >= 150) {
                        if (rankType == PicBookRankCategory.READ) {
    ...
    public void updateAllRank(PicBookRankCategory rankType, List<PicBookRankInfo> rankInfoList, Integer weekRange) {
        if (rankType == null || CollectionUtils.isEmpty(rankInfoList))
            return;
    ...
    @NoArgsConstructor
    @AllArgsConstructor
    @EqualsAndHashCode(of = {"rankType", "weekRange"})
    public class GeneratorKey {
        ...
        public String toString() {
            return "RT=" + rankType + ":WR=" + weekRange;
        }
    }
}
