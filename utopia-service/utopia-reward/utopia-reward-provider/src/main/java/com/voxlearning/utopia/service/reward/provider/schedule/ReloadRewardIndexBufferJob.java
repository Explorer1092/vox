...
import com.voxlearning.utopia.service.reward.impl.internal.InternalRewardIndexService;
...
@ScheduledJobDefinition(
        jobName = "ReloadRewardIndexBufferJob",
        cronExpression = "0 */5 * * * ?")
@ProgressTotalWork(100)
public class ReloadRewardIndexBufferJob extends ProgressedScheduleJob {
    ...
    protected void executeJob(long startTimestamp,
        ...
        long current = rewardIndexVersion.currentVersion();
        if (current != 1 && rewardLoader.getRewardIndexBuffer().getVersion() < current) {
            VersionedRewardIndexList data = internalRewardIndexService.loadVersionedRewardIndexList();
            rewardLoader.getRewardIndexBuffer().attach(data);
            //logger.info("Buffer [RewardIndex] reloaded, update version to {}.", data.getVersion());
        }
