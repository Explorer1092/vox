 ...
 */
@Install
final public class ResetTextBookManagementBuffer extends AbstractTestExecutionListener {
    @Override
    public void beforeTestMethod(TestContext testContext) throws Exception {
        ApplicationContextScanner.getInstance()
                .getBeansOfType(ApplicationContextScanner.Scope.ALL, TextBookManagementBuffer.Aware.class)
                .stream()
                .map(NamedBean::getBean)
                .forEach(TextBookManagementBuffer.Aware::resetTextBookManagementBuffer);
    }
}
