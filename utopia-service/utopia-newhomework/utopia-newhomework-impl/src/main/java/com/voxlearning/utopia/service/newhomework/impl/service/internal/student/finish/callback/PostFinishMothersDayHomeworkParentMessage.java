...
@Named
public class PostFinishMothersDayHomeworkParentMessage extends SpringContainerSupport implements PostFinishHomework {
    ...
    public void afterHomeworkFinished(FinishHomeworkContext context) {
        ...
        if (NewHomeworkType.MothersDay == context.getNewHomeworkType()) {
            ...
            if (!CollectionUtils.isEmpty(studentParentRefs)) {
                ...
                String content = "您的孩子" + studentDetail.fetchRealname() + "送您的母亲节礼物，祝您母亲节快乐~";
                Map<String, Object> jpushExtInfo = new HashMap<>();
                jpushExtInfo.put("s", ParentAppPushType.HOMEWORK_CHECK.name());
                jpushExtInfo.put("url", "/view/mobile/parent/motherdayhomework/index.vpage?hid=" + context.getHomeworkId());
