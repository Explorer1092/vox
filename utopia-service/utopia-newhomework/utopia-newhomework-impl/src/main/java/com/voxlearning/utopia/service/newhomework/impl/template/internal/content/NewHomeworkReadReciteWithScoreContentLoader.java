...
@Named
public class NewHomeworkReadReciteWithScoreContentLoader extends NewHomeworkContentLoaderTemplate {
    @Override
        ...
        try {
            ...
            if (CollectionUtils.isNotEmpty(readReciteContent)) {
                readReciteContent.forEach(e -> {
                    ...
                    String questionBoxId = SafeConverter.toString(e.get("id"));
                    ...
                    if (section != null && StringUtils.isNotBlank(section.getParentId())) {
                        ...
                        boxIdLessonIdMap.put(questionBoxId, book.getLessonId());

                        List<String> questionIdList = (List<String>) e.get("question_ids");
                ...
                //题的排行
                Map<String, Integer> qidRankMap = new HashMap<>();
                ...
                if (MapUtils.isNotEmpty(qidSentenceIdsMap)) {
                    for (Map.Entry<String, List<Long>> entry : qidSentenceIdsMap.entrySet()) {
                        ...
                        qidRankMap.put(questionDocId, sentenceIdRank.get(sentenceId));
                    }
                }
                            ...
                            for (String questionBoxId : questionBoxIds) {
                                ...
                                for (String questionId : questionIds) {
                                    ...
                                    if (newQuestion != null) {
                                        ...
                                        question.put("rankNumber", qidRankMap.get(questionId));
                                ...
                                comparator = comparator.thenComparingInt(a -> SafeConverter.toInt(a.get("rankNumber")));
                ...
                //按照课文排序
                Set<String> lessonIdSet = newReadReciteBOList.stream()
                        .filter(e -> e.getLessonId() != null)
                        .map(NewReadReciteBO::getLessonId)
                        .collect(Collectors.toSet());
                Map<String, NewBookCatalog> chineseLessonMap = newContentLoaderClient.loadBookCatalogByCatalogIds(lessonIdSet);
                newReadReciteBOList = newReadReciteBOList.stream()
                        .filter(e -> e.getLessonId() != null)
                        .filter(e -> chineseLessonMap.containsKey(e.getLessonId()))
                        .sorted(Comparator.comparingInt(a -> SafeConverter.toInt(chineseLessonMap.get(a.getLessonId()).getRank())))
                        .collect(Collectors.toList());
            ...
            logger.error("Load READ_RECITE_WITH_SCORE error:", ex);
        }
        return content;
    }
    ...
    public Map<String, Object> previewContent(Teacher teacher, String bookId, List<String> contentIdList) {
        ...
        try {
            ...
            logger.error("previewContent READ_RECITE_WITH_SCORE error:", ex);
        }
    ...
    private List<Map<String, Object>> processPreview(Set<String> questionBoxIds,
        ...
        if (CollectionUtils.isNotEmpty(questionBoxIds)) {
            ...
            for (String boxId : questionBoxIds) {
                ...
                List<NewQuestion> newQuestionList = questionLoaderClient.loadQuestionsIncludeDisabledAsList(questionIds);
                Map<String, NewQuestion> allQIdQuestionMap = newQuestionList
                        .stream()
                ...
                for (String qid : questionIds) {
                    ...
                    List<String> listenUrls = new ArrayList<>();
                    NewQuestion newQuestion = allQIdQuestionMap.get(qid);
                    if (newQuestion != null) {
                        List<NewQuestionOralDictOptions> options = newQuestion.getContent().getSubContents().get(0).getOralDict().getOptions();
                        if (CollectionUtils.isNotEmpty(options)) {
                            options.stream()
                    ...
                    questionMap.put("listenUrls", listenUrls);
