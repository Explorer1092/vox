...
@Spring
@Named
public class EventHandler_CleanupClazzZoneComment extends SpringContainerSupport implements ZoneEventHandler {

    @Inject private ClazzZoneCommentPersistence clazzZoneCommentPersistence;

    @Override
    public ZoneEventType getEventType() {
        return ZoneEventType.CLEANUP_CLAZZ_ZONE_COMMENT;
    }
    ...
    public void handle(JsonNode root) throws Exception {
        ...
        logger.info("We're going to cleanup all clazz zone comment(s) which created before {}", date);

        int count = clazzZoneCommentPersistence.getUtopiaSql()
                .withSql("DELETE FROM VOX_CLAZZ_ZONE_COMMENT WHERE CREATE_DATETIME<?")
                .useParamsArgs(date)
                .executeUpdate();

        logger.info("Total {} clazz zone comment(s) deleted", count);
    }
}
