...
@ServiceRetries
public interface ZoneProductService {

    @Async
    AlpsFuture<List<ClazzZoneProduct>> loadAllClazzZoneProductsFromDB();

    @Async
    AlpsFuture<ClazzZoneProduct> loadClazzZoneProductFromDB(Long id);

    @Async
    AlpsFuture<VersionedClazzZoneProductData> loadClazzZoneProductBufferData(long version);

    @NoResponseWait(dispatchAll = true, ignoreNoProvider = true)
    void reloadClazzZoneProductBuffer();
}
