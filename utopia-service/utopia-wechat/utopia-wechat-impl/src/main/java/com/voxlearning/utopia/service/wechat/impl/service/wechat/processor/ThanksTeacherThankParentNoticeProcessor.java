...
@Named
@NoArgsConstructor
public class ThanksTeacherThankParentNoticeProcessor extends AbstractNoticeProcessor {

    @Inject private WechatNoticePersistence wechatNoticePersistence;

    public WechatNoticeProcessorType type() {
        return WechatNoticeProcessorType.ThanksTeacherThankParentNotice;
    }

    @Override
    public void processWechat(Long userId, String openId, Map extensionInfo) {
        WechatNotice notice = new WechatNotice();
        notice.setMessageType(WechatNoticeType.TEMPLATE_THANKS_TEACHER_THANK_PARENT_NOTICE.getType());
