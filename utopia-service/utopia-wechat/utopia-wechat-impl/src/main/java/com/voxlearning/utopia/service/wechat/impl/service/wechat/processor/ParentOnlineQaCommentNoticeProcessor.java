...
@Named
@NoArgsConstructor
public class ParentOnlineQaCommentNoticeProcessor extends AbstractNoticeProcessor {
    ...
    protected void processWechat(Long userId, String openId, Map extensionInfo) {
        ...
        notice.setMessageType(WechatNoticeType.TEMPLATE_PARENT_ONLINE_QA_COMMENT.getType());
        notice.setState(WechatNoticeState.WAITTING.getType());
        notice.setUserId(userId);
        notice.setOpenId(openId);
        notice.setDisabled(false);
        Date now = new Date();
        notice.setSendTime(new Date(now.getTime() + 3600000));
        wechatNoticePersistence.persist(notice);
    }

    @Override
    public WechatNoticeProcessorType type() {
        return WechatNoticeProcessorType.ParentOnlineQaCommentNotice;
    }
}
