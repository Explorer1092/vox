...
@Named
public class RolePlayHandler extends UnitResultHandler {

    @Override
    protected ChipsUnitType type() {
        return ChipsUnitType.role_play_unit;
    }
    ...
    private int calListening(List<AIUserQuestionResultHistory> questionResultHistoryList) {
        List<AIUserQuestionResultHistory> list = questionResultHistoryList.stream()
                .filter(e -> e.getQuestionType() == ChipsQuestionType.video_conversation || e.getQuestionType() == ChipsQuestionType.video_dialogue || e.getQuestionType() == ChipsQuestionType.role_play_practice)
                .filter(h -> h.getScore() != null && h.getScore() > 0 )
                .collect(Collectors.toList());
        if (CollectionUtils.isEmpty(list)) {
            return 59;
    ...
    private int calPronunciation(List<AIUserQuestionResultHistory> questionResultHistoryList) {
        List<AIUserQuestionResultHistory> list = questionResultHistoryList.stream()
                .filter(e -> e.getQuestionType() == ChipsQuestionType.role_play_practice)
