...
@Named
public class TopicLeaningHandler extends UnitResultHandler {

    @Override
    protected ChipsUnitType type() {
        return ChipsUnitType.topic_learning;
    }
    ...
    private int calListening(List<AIUserQuestionResultHistory> questionResultHistoryList) {
        List<AIUserQuestionResultHistory> list = questionResultHistoryList.stream()
                .filter(e -> e.getQuestionType() == ChipsQuestionType.video_conversation || e.getQuestionType() == ChipsQuestionType.video_dialogue)
    ...
    private int calPronunciation(List<AIUserQuestionResultHistory> questionResultHistoryList) {
        List<AIUserQuestionResultHistory> list = questionResultHistoryList.stream()
                .filter(e -> e.getQuestionType() == ChipsQuestionType.video_conversation || e.getQuestionType() == ChipsQuestionType.video_dialogue || e.getQuestionType() == ChipsQuestionType.word_repeat || e.getQuestionType() == ChipsQuestionType.sentence_repeat)
    ...
    private int calExpress(List<AIUserQuestionResultHistory> questionResultHistoryList) {
        ...
        int score = new BigDecimal(list.stream().mapToInt(AIUserQuestionResultHistory::getScore).sum())
                .divide(new BigDecimal(list.size()), 2, BigDecimal.ROUND_HALF_UP)
                .multiply(new BigDecimal(0.5D))
                .intValue();
        int complete = new BigDecimal(list.stream().mapToInt(AIUserQuestionResultHistory::getCompleteScore).sum())
                .divide(new BigDecimal(list.size()), 2, BigDecimal.ROUND_HALF_UP)
                .multiply(new BigDecimal(0.5D))
                .intValue();
