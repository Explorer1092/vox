...
@DocumentTable(table = "VOX_NEW_TERM_PLAN_ACTIVITY")
@UtopiaCacheExpiration(policy = CachedObjectExpirationPolicy.today)
@UtopiaCacheRevision("20190218")
@NoArgsConstructor
public class NewTermPlanActivity extends AbstractDatabaseEntity implements CacheDimensionDocument {

    private static final long serialVersionUID = 6989490205718509539L;

    public NewTermPlanActivity(Long userId) {
        this.id = userId;
    }
    ...
    public String[] generateCacheDimensions() {
        return new String[]{
                CacheKeyGenerator.generateCacheKey(NewTermPlanActivity.class, this.id)
        };
    }
}
