 ...
 */
@Named
public class ParentPicListenLotteryWrapper extends AbstractCampaignWrapper {

    private static final String lotteryChanceKey = "PARENT_PICLISTEN_LOTTERY_FREE_CHANCE:";
    private static final int activityExpiration = (int) (DateUtils.stringToDate("2017-06-30 23:59:59").getTime() / 1000);
    ...
    public MapMessage doLottery(CampaignType campaignType, final User user, LotteryClientType clientType) {
        ...
        try {
            ...
            // 记录最近的用户中奖信息
            if(SafeConverter.toInt(winLottery.get("awardId")) > 2){
                addRecentCampaignLotteryResult(user, lotteryResult, campaignType.getId());
            }
            if (SafeConverter.toInt(winLottery.get("awardId")) <= 5) {
                // 添加缓存大奖纪录
                if(SafeConverter.toInt(winLottery.get("awardId")) <= 2){
                    addCampaignLotteryResultBig(user, lotteryResult, campaignType.getId(), activityExpiration);
                }
                // 记录大奖记录
            ...
            switch (SafeConverter.toInt(winLottery.get("awardId"))) {
                case 6:
                    couponId = "5923a90577748775cc6d4a0b";
                    if(RuntimeMode.current().ge(Mode.STAGING)){
                        couponId = "5922bbd95d9d4b08341367a0";
                    }
                    break;
                case 7:
                    couponId = "5923a955e92b1b7e24fcb2be";
                    if(RuntimeMode.current().ge(Mode.STAGING)){
                        couponId = "5922bb61e485aba6976c6d7c";
                    }
                    break;
                case 8:
                    couponId = "5923a99de92b1b7e24fcb2ca";
                    if(RuntimeMode.current().ge(Mode.STAGING)){
                        couponId = "5922ba0c5d9d4b0834135f5e";
                    }
                    break;
                default:
            }
            ...
            logger.error("PARENT PIC LISTEN LOTTERY ERROR, {}", ex.getMessage());
    ...
    public Long addLotteryChance(Long userId, int delta) {
        ...
        if (ret == null) {
            logger.error("Failed increase {} parent pic listen lottery free chance with delta {}", userId, delta);
            return 0L;
        }
        return ret;
    }
