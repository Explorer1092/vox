...
@ScheduledJobDefinition(
        jobName = "新学期收心计划发运营消息",
        jobDescription = "新学期收心计划发运营消息",
        disabled = {Mode.STAGING, Mode.TEST, Mode.DEVELOPMENT},
        cronExpression = "0 0 2 * * ?",
        ENABLED = false
)
public class NewTermPlanMsgJob extends ScheduledJobWithJournalSupport {
    ...
    @AlpsPubsubPublisher(topic = "utopia.campaign.new_term_plan_msg.topic", messageEncodeMode = MessageEncodeMode.PLAIN_TXT)
    ...
    private void jobEnd() {
        emailServiceClient.createPlainEmail()
                .body("正常结束")
                .subject("【" + RuntimeMode.current() + "】" + "新学期收心计划发运营消息")
                .to("junbao.zhang@17zuoye.com")
                .send();
    }

    private List<Long> fetchMoreUserList(Long userId, Integer pageSize) {
        return utopiaSql.withSql("SELECT ID FROM VOX_NEW_TERM_PLAN_ACTIVITY WHERE ID > " + userId + " LIMIT " + pageSize)
