...
@ScheduledJobDefinition(
        jobName = "同步七鱼客服聊天记录",
        jobDescription = "同步七鱼客服聊天记录，每天凌晨2:45运行一次",
        disabled = {Mode.DEVELOPMENT, Mode.STAGING, Mode.UNIT_TEST, Mode.TEST},
        cronExpression = "0 45 2 * * ?"
)
public class AutoSyncQiyuMessageJob extends ScheduledJobWithJournalSupport {
    ...
    private static final String RESOURCE_PATH = "/tmp/qiyu";
    private static final int INTERVAL = 1;
    private static final int SUCCESS_CODE = 200;
    private static final String CK = "qiyu_session_start_time";
...
//        downloadAndSave("open.api.ex.s.100964", null);
        Date ckValue = CacheSystem.CBS.getCache("storage").load(CK);
        Date startTime = DateUtils.stringToDate(DateUtils.getYesterdaySqlDate(), DateUtils.FORMAT_SQL_DATE);
        Date cacheStartTime = SafeConverter.toDate(ckValue, startTime);
        Date endTime = DateUtils.stringToDate(DateUtils.getTodaySqlDate(), DateUtils.FORMAT_SQL_DATE);
        while(cacheStartTime.before(endTime)){
            try {
                Date tmpEnd = DateUtils.addDays(cacheStartTime, INTERVAL);
                if(tmpEnd.after(endTime))
                    tmpEnd = endTime;
                Date exportStart = cacheStartTime;
                Date exportEnd = tmpEnd;
                cacheStartTime = tmpEnd;
                ...
                logger.error("qiyu session job error {}", e);
                return;
            }
        }
    ...
    private boolean exportData(Date startTime, Date endTime){
        ...
        while(true){
            try {
                ...
                logger.error("AutoSyncQiyuMessageJob error:{}", e);
                return false;
            }
        }
        return false;
