...
@ScheduledJobDefinition(
        jobName = "薯条英语开课后每天早上提醒",
        jobDescription = "每天6点半执行",
        disabled = {Mode.UNIT_TEST, Mode.STAGING},
        cronExpression = "0 30 6 * * ?"
)
@ProgressTotalWork(100)
public class AutoSendCourseDailyBeginNotifyJob extends ScheduledJobWithJournalSupport {
    ...
    @AlpsQueueProducer(queue = "utopia.chips.course.daily.begin.notify.queue")
    private MessageProducer courseDailyBeginNotifyMessageProducer;
    ...
    protected void executeScheduledJob(JobJournalLogger jobJournalLogger,
        ...
        orderProductList = orderProductList.stream()
                ...
                .filter(e -> {
                    ...
                    Date beginDate = Optional.ofNullable(timetable).map(ChipsEnglishProductTimetable::getBeginDate).orElse(null);
                    Date endDate = Optional.ofNullable(timetable).map(ChipsEnglishProductTimetable::getEndDate).orElse(null);
        ...
        for(OrderProduct orderProduct : orderProductList) {
            for(Long id : userIds) {
                ...
                message.put("U", id);
                courseDailyBeginNotifyMessageProducer.produce(Message.newMessage().withPlainTextBody(JsonUtils.toJson(message)));
                if (index % 100 == 0) {
                    try {
                        Thread.sleep(1000);
