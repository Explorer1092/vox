...
@ScheduledJobDefinition(
        jobName = "薯条英语开课提醒",
        jobDescription = "每天8点执行",
        disabled = {Mode.UNIT_TEST, Mode.STAGING},
        cronExpression = "0 0 8 * * ?"
)
@ProgressTotalWork(100)
public class AutoSendChipsCourseBeginNotifyJob extends ScheduledJobWithJournalSupport {
    ...
    @AlpsQueueProducer(queue = "utopia.chips.course.begin.notify.queue")
    ...
    protected void executeScheduledJob(JobJournalLogger jobJournalLogger,
        ...
        orderProductList = orderProductList.stream()
                .filter(e -> {
                    ...
                    return timetable.getBeginDate().after(now) && timetable.getBeginDate().before(nextDay);
                })
                .collect(Collectors.toList());
