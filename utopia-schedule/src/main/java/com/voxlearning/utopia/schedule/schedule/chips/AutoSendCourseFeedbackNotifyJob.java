...
public class AutoSendCourseFeedbackNotifyJob extends ScheduledJobWithJournalSupport {
    ...
    @AlpsQueueProducer(queue = "utopia.chips.course.end.feedback.queue")
    private MessageProducer courseEndFeedbackMessageProducer;
    ...
    protected void executeScheduledJob(JobJournalLogger jobJournalLogger,
        ...
        List<OrderProduct> orderProductList = userOrderLoaderClient.loadAllOrderProductIncludeOffline().stream()
                ...
                .filter(e -> {
                    ...
                    return SafeConverter.toBoolean(map.get("short"));
                })
        ...
        orderProductList = orderProductList.stream()
                .filter(e -> {
                    ...
                    if (timetable == null || timetable.getEndDate() == null) {
                        return false;
                    }
                    return timetable.getEndDate().before(now) && timetable.getEndDate().after(yesterday);
                })
                .collect(Collectors.toList());
        ...
        for (OrderProduct orderProduct : orderProductList) {
            for (Long id : userIds) {
                ...
                message.put("U", id);
                courseEndFeedbackMessageProducer.produce(Message.newMessage().withPlainTextBody(JsonUtils.toJson(message)));
                if (index % 100 == 0) {
                    try {
                        Thread.sleep(1000);
