...
@ScheduledJobDefinition(
        jobName = "薯条英语课程毕业证书提醒",
        jobDescription = "每天18点执行",
        disabled = {Mode.UNIT_TEST, Mode.STAGING},
        cronExpression = "0 0 18 * * ?"
)
@ProgressTotalWork(100)
public class AutoGenGraduationCertificateNotifyJob extends ScheduledJobWithJournalSupport {
    ...
    @AlpsQueueProducer(queue = "utopia.chips.course.end.certificate.queue")
    private MessageProducer courseEndCertificateMessageProducer;
    ...
    protected void executeScheduledJob(JobJournalLogger jobJournalLogger,
        ...
        orderProductList = orderProductList.stream()
                .filter(e -> {
                    ...
                    // EndDate
                    return DateUtils.isSameDay(timetable.getEndDate(), now);
                })
                .collect(Collectors.toList());
        ...
        for (OrderProduct orderProduct : orderProductList) {
            for (Long id : userIds) {
                ...
                message.put("U", id);
                courseEndCertificateMessageProducer.produce(Message.newMessage().withPlainTextBody(JsonUtils.toJson(message)));
                if (index % 100 == 0) {
                    try {
                        Thread.sleep(500);
